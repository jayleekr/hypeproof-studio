...
try {
  ...
  await page.locator('#ops-help').screenshot({ path: path.join(out, 'pages-01-older-window.png') }); await page.setViewportSize({ width: 390, height: 844 }); if (await page.locator('#ops-detail').isVisible()) await page.keyboard.press('Escape'); assert.ok(await page.evaluate(() => document.documentElement.scrollWidth <= innerWidth), 'no horizontal overflow at 390 with two window actions'); await page.locator('#ops-help').screenshot({ path: path.join(out, 'pages-01-older-window-390.png') }); await page.setViewportSize({ width: 1280, height: 900 });
