...
await page.locator('#ops-pick-items').filter({ hasText: /A1 · .* — 서버 검증됨/ }).waitFor({ timeout: 120000 }).catch(async (e) => { throw Error('A1 did not verify: ' + (await page.locator('#ops-pick-state').innerText()) + ' | ' + (await page.locator('#ops-pick-items').innerText()), { cause: e }); });
await page.locator('#ops-pick-state').filter({ hasText: /결과 확정/ }).waitFor({ timeout: 30000 }); await page.locator('#ops-pick-state').scrollIntoViewIfNeeded(); await page.screenshot({ path: path.join(out, '02-collected.png'), fullPage: true });
