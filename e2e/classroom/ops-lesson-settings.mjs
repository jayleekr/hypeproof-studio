...
try {
  ...
  await host2({ type: 'streamStart', streamId: turn1.streamId ?? 'stream-1', messageId: 'm-1' });
  // Enter parks a message only while the screen itself shows a running turn; pressing it before the state landed SENT it (a race of this test, seen 1 run in 3).
  await app2.getByRole('button', { name: 'Stop' }).waitFor(); await in2.fill('예약한 입력'); await in2.press('Enter');
  ...
  await app2.waitForFunction(() => document.activeElement?.tagName === 'TEXTAREA', null, { timeout: 5000 }) /* the component hands focus back on the next tick — asserting it at once was a race of this test */; let hops = 0; while (hops++ < 40 && !(await app2.evaluate(() => document.activeElement?.hasAttribute('data-inbox-undo')))) await app2.keyboard.press('Shift+Tab'); assert.ok(hops < 40, 'undo is reachable with Shift+Tab alone'); await app2.keyboard.press('Enter'); assert.equal(await in2.inputValue(), '내가 쓰던 초안'); assert.equal(await app2.locator('.hps-attachment').count(), 2);
