...
const device = async (seat, n, executors) => { const caps = ['observe', 'commands', 'retry_evidence_upload'], c = (await local.pair(seat, 1, n, caps)).conn.json, inst = local.instance(n, caps); await consentOf(c);
  ...
  const loop = ops.startOpsSync({ outbox: await ops.OpsOutbox.open(memory(), c.grant_id, 'stream-' + seat, () => Date.now(), () => `event-u1b-${String(++eid).padStart(6, '0')}`), appInstanceId: inst.app_instance_id, capabilities: caps, commands: runner, sample: () => ({ idle_ms: 0, status: 'idle' }),
