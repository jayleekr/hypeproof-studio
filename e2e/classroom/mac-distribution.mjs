...
try {
  ...
  await chat.evaluate("document.querySelector('details.hp-inbox[data-inbox-generation]').open=false"); const again = await chat.evaluate(INBOX(true)); assert.equal(again.cards[0].body, V1, 'opened again'); await wait(async () => /새 자료/.test((await chat.evaluate(INBOX(false))).summary) ? null : true, '"new" goes away once it was opened', 15000);
