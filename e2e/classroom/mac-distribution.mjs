...
try {
  ...
  await page.locator('#ops-dist-title').fill('오늘 수업 안내'); await page.locator('#ops-dist-body').fill(V1); await page.locator('#ops-dist-save').click(); await page.locator('#ops-dist-saved').filter({ hasText: '저장했습니다' }).waitFor();
  ...
  assert.match(work2.summary, /끝난 수업의 자료/, 'not connected after the restart in this harness: the material is still readable'); await connectSeat();
  ...
  const V2 = '오늘 수업은 3시 10분에 끝납니다. (수정)\n끝나기 10분 전에 작업을 저장하세요.'; await page.locator('#ops-dist-body').fill(V2); await page.locator('#ops-dist-save').click(); await page.locator('#ops-dist-saved').filter({ hasText: '2번째 판' }).waitFor();
  ...
  await page.locator('#ops-dist-revoke').click(); await page.locator('#ops-dist-revoke-go').click(); await page.locator('#ops-dist-state').filter({ hasText: '이 배포는 회수됨' }).waitFor(); await new Promise((r) => setTimeout(r, 12000));
  await page.locator('#ops-dist-refresh').click(); await page.waitForTimeout(500); assert.match(await T('ops-dist-items'), /같은 판의 다른 배포가 보관함을 유지/); assert.deepEqual((await chat.evaluate(INBOX(true))).cards.map((c) => [c.body, c.revision]), [[V2, '2']], 'the other v2 run still permits what is shown'); await page.screenshot({ path: path.join(out, '09-instructor-first-withdrawal-covered.png'), fullPage: true });
  ...
  await runsShown.filter({ hasNotText: '회수됨' }).getByRole('button').click(); await page.locator('#ops-dist-state').filter({ hasNotText: '이 배포는 회수됨' }).waitFor(); await page.locator('#ops-dist-revoke').click(); await page.locator('#ops-dist-revoke-go').click(); await page.locator('#ops-dist-state').filter({ hasText: '이 배포는 회수됨' }).waitFor();
