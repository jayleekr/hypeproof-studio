...
try {
  ...
  await page.setViewportSize({ width: 1280, height: 720 }); await page.evaluate(() => scrollTo(0, 0)); const firstScreen = await page.evaluate(() => { const top = (s) => Math.round(document.querySelector(s).getBoundingClientRect().top + scrollY); return { select: top('#ops-select-all'), action: top('#ops-dist-preview'), first_student_row: top('#ops-seats .ops-seat'), composer_open: document.querySelector('#ops-dist').open, history_open: document.querySelector('#ops-dist-history').open }; });
