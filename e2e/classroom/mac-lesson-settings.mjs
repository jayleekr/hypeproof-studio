...
const turns = () => db('SELECT substr(turn_id,1,8) AS turn,binding_seq,version,requests,closed_at IS NOT NULL AS closed,close_outcome,first_dispatched_at IS NOT NULL AS dispatched,first_completed_at IS NOT NULL AS completed,last_failure_kind FROM classroom_lesson_turns WHERE student_id=? ORDER BY admitted_at', seats[0].student_id);
...
try {
  ...
  const allTurns = turns(), result = { schema: 'hps-classroom-mac-lesson-settings/2', at: new Date().toISOString(), source_sha: head, extension_source_sha: manifest.extension.source_sha, extension_bundles: manifest.extension.bundles, shell: manifest.shell, agent_sdk: { version: manifest.agent_sdk.version, binary_sha256: manifest.agent_sdk.binary.sha256 }, real_seat: 'A1', synthetic_seat: 'A2',
