...
try {
  ...
  for (const [button, expected] of [['ops-select-all', /선택 30 \/ 전체 30석 \(명단 1차\) · 기기 연결됨 11 · 기기 연결 없음 19/], ['ops-select-online', /선택 11 \/ 전체 30석/], ['ops-select-offline', /선택 19 \/ 전체 30석 .* 기기 연결됨 0/], ['ops-select-fault', /선택 (9|1\d|2\d) \/ 전체 30석/]]) { if (button !== 'ops-select-fault') await page.locator('#ops-pick-by > summary').click(); /* G1: all/online/offline sit under ‘다른 조건으로 선택’ */ await page.locator('#' + button).click(); assert.match(await selection(), expected, button); }
  ...
  assert.match(await page.locator('#ops-pick-impact').innerText(), /선택 3명 중 1명에게 회수를 요청합니다\(종류는 아래에서 고름\)\. 제외 2명.*선택하지 않은 27명에게는 아무것도 요청·저장하지 않습니다\. 회수만 하며 평가 초안·발송은 시작하지 않습니다/);
  ...
  await page.locator('#ops-pick-kind-record').check(); await page.locator('#ops-pick-go').dblclick(); // a double click is one request
  ...
  assert.match(await page.locator('#ops-pick-state').innerText(), /선택 회수\(수업 기록 전체\) · 대상 3명 · 서버 검증됨 1 · 기기 수신 확인 전 0 · 전송·검증 진행 중 0 · 재전송 대기 0 · 최종 거부 0 · 전달 안 됨 1 · 결과 미확인 0 · 유예 종료 0 · 제외 1 · 선택하지 않은 27명은 요청 없음/);
  ...
  const pickKeys = [...local.r2.keys()].filter((k) => k.includes(pickBatches[0].id)); assert.equal(pickKeys.length, 3, 'one session = one /3 part: meta, index and the asked lines (U1b)'); assert.ok(pickKeys.every((k) => k.includes('/student-01/')), 'objects only for the selected, consenting, connected learner: ' + pickKeys.join(' '));
