...
try {
  ...
  await build({ entryPoints: [path.join(repo, 'extensions/hypeproof-chat/src/classroomOpsHost.ts')], outfile: bundled, bundle: true, platform: 'node', format: 'cjs', logLevel: 'silent',
    plugins: [{ name: 'vscode-stub', setup(b) { b.onResolve({ filter: /^vscode$/ }, () => ({ path: 'vscode', namespace: 'stub' })); b.onLoad({ filter: /.*/, namespace: 'stub' }, () => ({ loader: 'js', contents: 'const w={showInputBox:async()=>globalThis.__hpsTicket,showInformationMessage:async(_m,o,...items)=>(o&&o.modal?items[0]:undefined),showWarningMessage:async()=>undefined,showQuickPick:async()=>undefined};export const window=w;export const commands={executeCommand:async()=>undefined};export const workspace={getConfiguration:()=>({get:()=>"https://service.test/v1"})};' })); } }] });
