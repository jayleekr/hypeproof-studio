...
try {
  ...
  await palette(win, 'HypeProof: HTML 미리보기 (옆 패널)'); const tabs = async () => (await (await realFetch('http://127.0.0.1:' + debugPort + '/json/list')).json()).filter((t) => /^http:\/\/127\.0\.0\.1:\d+\/page\.html/.test(t.url));
  const tab = await wait(async () => (await tabs())[0], 'the learner\'s preview tab is open'); const previewUrl = tab.url; assert.match(await (await realFetch(previewUrl)).text(), /SYNTHETIC LEARNER PAGE U4/);
  ...
  // ── R7 token: issuing is not receiving. The old issue re-checked → 문제 남음; the re-issued token in the app → 해결 확인. ──
  const recheck = (await I.act('A1', '연결 다시 확인', /해결 확인 1/)).replace(/\s+/g, ' '); assert.match(recheck, /수업 정보 재확인됨 → 문제 해결 확인 · 기존 발급분을 다시 확인함 \(재발급 아님\)/);
  const reissued = await invite(seats[0].student_id); assert.notEqual(reissued, token); await wait(async () => (await seatNow('A1')).token?.app_verified === 'other_token', 'the board shows the app is on another issue'); assert.equal((await seatNow('A1')).recommended.action, 'refresh_connection');
  const oldLine = (await I.act('A1', '연결 다시 확인', /문제 남음 1/)).replace(/\s+/g, ' '); assert.match(oldLine, /→ 문제 남음 — 원인: 새로 발급한 토큰이 학생 앱에 없음 · 학생 앱이 확인한 것은 이전 발급분.*다음: 새 토큰을 학생에게 전달/); await I.shot('r7-old-issue-still-in-app.png');
  // the learner receives the new token: written to the dev token file, the COPY restarted (labelled — the start-page entry is not run)
  writeFileSync(tokenPath, reissued, { mode: 0o600 }); try { app.kill(); } catch {} await sleep(3000); launch(); win = await attach(); chat = await enterWork(); await connectSeat(win);
  ...
  const result = { schema: 'hps-classroom-mac-recovery/1', at: new Date().toISOString(), source_sha: head, extension_source_sha: manifest.extension.source_sha, extension_bundles: manifest.extension.bundles, shell: manifest.shell, agent_sdk: { version: manifest.agent_sdk.version, binary_sha256: manifest.agent_sdk.binary.sha256, route_seen: '/v1/messages' }, how: 'instructor = clicks on the Chalk page (visible Chromium); learner = real Studio window over the debugging port (real mouse input for webview controls)',
    ...
    made_here: ['accounts, class and tokens (synthetic)', 'model provider (scripted stand-in; "provider down" = it answers 500)', '"/v1/health does not answer OK" at the local HTTP front', 'the learner page moved away and back by the runner', 'seat A2 (real device client in this process), A3 (never connected), A4 (no command capability)', 're-issued token delivered through the dev token file + restart'],
    not_run: ['real model', 'school network / real outage', 'Windows', 'several physical devices', 'staging or production D1/R2', 'mail', 'Keychain-backed installed app', 'typing a re-issued token into the start page', 'preview server restart on a new port in a real window', 'a multi-request turn crossing a pause in a real window'],
...
function rec_shared(cls) { return ['provider_rate_limit', 'provider_5xx', 'network', 'class_not_open', 'budget_limit'].includes(cls); }
