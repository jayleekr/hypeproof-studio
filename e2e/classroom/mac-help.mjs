...
try {
  ...
  // Hours:minutes only — this runner's ICU writes "AM 09:15" where the app writes "오전 09:15" (attempt 1 failed on that, not on the value).
  const hm = (d) => [d.getHours() % 12 || 12, d.getMinutes()].map((n) => String(n).padStart(2, '0')).join(':'); assert.ok(preview.expiry.includes(hm(new Date(preview.end * 1000))), 'the time shown is the signed end ' + hm(new Date(preview.end * 1000)) + ': ' + preview.expiry);
