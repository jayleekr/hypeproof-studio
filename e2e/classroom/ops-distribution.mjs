...
try {
  ...
  assert.doesNotMatch(await T('ops-dist-note'), /확인만 했습니다/, 'once confirmed, the confirm-step notice is gone'); assert.match(await T('ops-last-what'), /배포 접수\(전달 아님\) → A1, A3/, 'the accepted distribution is named with its targets in the previous-run line'); assert.match(await T('ops-dist-state'), /대상 2명 · 보관함 반영 0 · 접수 2/, 'recorded is not delivered');
