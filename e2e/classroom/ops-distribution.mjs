...
try {
  ...
  const write = async (title, body, kind = 'notice') => { await page.locator('#ops-dist-kind').selectOption(kind).catch(() => {}); await page.locator('#ops-dist-title').fill(title); await page.locator('#ops-dist-body').fill(body); await page.locator('#ops-dist-save').click(); await page.locator('#ops-dist-saved').filter({ hasText: '저장했습니다' }).waitFor(); };
  ...
  await connect(local.teacherToken); assert.equal(await page.locator('#ops-dist').isVisible(), false, 'observe, coach, collect, command, deliver … none of them shows "공지·자료 보내기"'); assert.equal(await page.locator('#ops-pick-collect').isVisible(), true, 'while what that token DOES hold is there');
  ...
  await connect(X); assert.equal(await page.locator('#ops-dist').isVisible(), true); assert.equal(await page.locator('#ops-dist-preview').isDisabled(), true, 'nothing is selected and nothing is saved: nothing can be sent');
  await write('다음 시간 준비물', '노트북 충전기를 가져오세요.\n<script>alert(1)</script>'); assert.match(await T('ops-dist-saved'), /1번째 판 .* 아직 아무에게도 보내지 않았습니다/); assert.equal(runs().length, 0); assert.equal(await page.locator('#ops-dist-preview').isDisabled(), true, 'saved, but the default target is still the empty selection');
  ...
  await page.locator('#ops-dist-revoke').click(); assert.match(await T('ops-dist-revoke-impact'), /같은 판을 보낸 다른 배포가 남아 있지 않을 때만 카드가 내려갑니다.*이미 본 내용은 되돌릴 수 없습니다/s); await page.locator('#ops-dist-revoke-go').click(); await page.locator('#ops-dist-state').filter({ hasText: '이 배포는 회수됨' }).waitFor(); await tickAll();
  assert.match(await T('ops-dist-items'), /보관함: 이 배포는 회수됨 · 같은 판의 다른 배포가 보관함을 유지/); assert.deepEqual(await dev.A1.cards(), [['다음 시간 준비물', 2, '본문을 확인 뒤에 고쳤다']], 'another run of v2 still permits the card');
  ...
  await page.locator('#ops-dist-revoke').click(); await page.locator('#ops-dist-revoke-go').click(); await page.locator('#ops-dist-state').filter({ hasText: '이 배포는 회수됨' }).waitFor(); await tickAll(); await page.locator('#ops-dist-refresh').click(); await page.waitForTimeout(300);
  ...
  await page.locator('#ops-dist-new').click(); await write('자료 회수 시험', '곧 회수할 글'); await page.locator('#ops-select-none').click(); await box('A2').check(); cool(); await page.locator('#ops-dist-preview').click(); await page.locator('#ops-dist-confirm').waitFor(); await page.locator('#ops-dist-go').click(); await page.locator('#ops-dist-items').filter({ hasText: /^A2 · student-b/ }).waitFor(); await tickAll(); await waitState(/모두 반영/);
  ...
  local.db.prepare("UPDATE sessions SET ended_at='2026-01-01T00:00:00Z' WHERE id=?").run(local.run); await page.locator('#ops-dist-new').click(); await page.locator('#ops-dist-title').fill('끝난 뒤'); await page.locator('#ops-dist-body').fill('x'); await page.locator('#ops-dist-save').click(); await page.locator('#ops-dist-saved').filter({ hasText: '이 수업은 끝났습니다' }).waitFor();
  ...
  const built = await esbuild.build({ stdin: { contents: `import { createRoot } from "react-dom/client"; import { createElement } from "react"; import { InstructorInbox } from "./webview-ui/src/InstructorInbox.tsx"; window.mount = (inbox) => { window.posted = []; createRoot(document.getElementById("root")).render(createElement(InstructorInbox, { inbox, post: (m) => window.posted.push(m) })); };`, resolveDir: path.join(repo, 'extensions/hypeproof-chat'), loader: 'tsx' }, bundle: true, write: false, format: 'iife', jsx: 'automatic', nodePaths: [path.join(repo, 'extensions/hypeproof-chat/webview-ui/node_modules')], define: { 'process.env.NODE_ENV': '"production"' }, logLevel: 'silent' });
  ...
  assert.deepEqual(requests.filter((u) => !u.startsWith('about:') && !u.startsWith('data:')), [], 'nothing was fetched, opened or downloaded by showing it'); assert.deepEqual(await learner.evaluate(() => window.posted), [{ type: 'inboxOpen', objectId: '99999999-9999-4999-8999-999999999999', generation: 7 }], 'opening is told to the host (to stop saying "new") and nothing else happens');
