...
export async function budgetFixture(){
 const h=await accessHarness(),cohort='studio-model-practice',profile=cohort,issuer='synthetic-teacher';
 ...
 const upstream=[],realFetch=globalThis.fetch;
 const serviceFetch=async request=>{const ctx=makeCtx(),r=await service.fetch(request,h.env,ctx);const response=new Response(r.body,r);response.done=()=>ctx.settle();return response;};
 globalThis.fetch=async(input,init)=>{
  ...
  if(['api.openai.com','api.anthropic.com'].includes(url.hostname)){
   const body=JSON.parse(init.body),isOpenAI=url.hostname==='api.openai.com';upstream.push({provider:isOpenAI?'openai':'anthropic',model:body.model,request_state:h.db.prepare("SELECT COUNT(*) n FROM usage_attempt_costs WHERE execution_state='sent'").get().n});
 ...
 return{...h,cohort,profile,token,teacher,otherTeacher,root,contract,api,upstream,serviceOrigin:serviceServer.origin,chalkOrigin:chalkServer.origin,
