...
try {
 ...
 if(models){await page.locator('#models-load').click();await page.locator('#models-status').filter({hasText:'사용 가능한 모델 2개'}).waitFor();await page.locator('#model-mode').selectOption('choice');await page.locator('#model-default').selectOption('hypeproof-default');for(const width of [390,1280]){await page.setViewportSize({width,height:900});assert.ok(await page.evaluate(()=>document.documentElement.scrollWidth<=innerWidth));const out=process.env.HPS_CHALK_AUTHORING_OUT||'test-results/chalk-authoring';mkdirSync(out,{recursive:true});await page.locator('#editor').screenshot({path:out+'/model-choice-'+width+'.png'});}}
 ...
 if(models){assert.equal(await page.locator('#model-mode').inputValue(),'choice');await page.locator('#models-load').click();await page.locator('#models-status').filter({hasText:'사용 가능한 모델 2개'}).waitFor();assert.equal(await page.locator('#model-default').inputValue(),'hypeproof-default');}
 ...
 if(models){await second.locator('#models-load').click();await second.locator('#models-status').filter({hasText:'사용 가능한 모델 2개'}).waitFor();await second.locator('#model-mode').selectOption('fixed');await second.locator('#model-default').selectOption('hypeproof-fast');}
 ...
 if(models){const frozen=JSON.parse(await page.locator('#version-view').innerText());assert.equal(frozen.module.content.model.default,'hypeproof-default');assert.equal(frozen.module.content.model.allowed.length,2);assert.equal(frozen.module.content.model.binding.choices[0].id,'claude-sonnet-4-6');console.log('PASS Chalk model choice save/reopen/freeze; later fixed-model draft preserves original version');}
