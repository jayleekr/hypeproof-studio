...
test('course effort reaches the real model and survives UI transitions', async () => {
  ...
  try {
    ...
    for (const width of [390,1280]) {
      ...
      await ctx.win.screenshot({path:join(out,'effort-'+width+'.png')});
    ...
    await ctx.win.screenshot({path:join(out,'unsupported-model-record.png')});
    ...
    await entry.getByLabel('수업 참여 코드',{exact:true}).fill(readFileSync(process.env.HPS_E2E_TOKEN_FILE+'.fixed','utf8').trim());
    ...
    await ctx.win.screenshot({path:join(out,'fixed-course.png')});
