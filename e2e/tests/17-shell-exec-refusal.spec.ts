...
const REFUSES_OR_REDIRECTS =
  /실행.{0,5}(할 수 없|드릴 수 없|해 ?드릴 수 없|불가|어렵|안 ?돼)|돌릴 수 없|돌릴 수가 없|실행할 수 없|할 수 없어요|터미널.{0,10}(직접|접근)|직접.{0,5}(터미널|실행|돌리|입력|붙여)|셸 실행|외부 명령|이 워크숍|이 세션|이 채팅|코치 역할|코치라서|검색|치과|환자/;
