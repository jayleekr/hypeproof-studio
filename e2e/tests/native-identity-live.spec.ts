...
test('individual codes isolate history and observation in the same actual workspace',async()=>{
 ...
 try{
  ...
  const connect=async(code:string)=>{await (await chatFrame(ctx.win)).getByRole('button',{name:'수업 연결',exact:true}).click();const entry=await startFrame(ctx.win);await entry.getByRole('button',{name: /^(?:다른 활동 선택|다른 수업에 연결)$/}).click();if(await entry.getByRole('button',{name:'수업에 참여하기',exact:true}).isVisible())await entry.getByRole('button',{name:'수업에 참여하기',exact:true}).click();await entry.getByLabel('수업 참여 코드',{exact:true}).fill(code);await entry.getByRole('button',{name: /^(?:코드 확인하기|수업 확인하기)$/}).click();await expect(entry.locator('.studio-course')).toBeVisible();await entry.getByRole('button',{name: /^(?:이어서 하기|수업 시작하기)$/}).click();};
