...
export async function verifyModels({app,window,findContext,cases,out,live,upstream,gatewayCalls}) {
 const checks=[];
 ...
 try {
  ...
  for(const width of [390,1280]){
   for(let n=0;n<4;n++){const current=await chat.evaluate('innerWidth');if(current===width)break;const b=await window.locator('.part.sidebar').boundingBox();await window.mouse.move(b.x+b.width,b.y+b.height/2);await window.mouse.down();await window.mouse.move(b.x+b.width+width-current,b.y+b.height/2,{steps:10});await window.mouse.up();await window.waitForTimeout(200);}
  ...
  if(live){
   const send=async(prompt,expected,switchDuring)=>{
    ...
    assert.ok(calls.some(c=>c.status===200&&c.model===expected),'selected model was not observed upstream');
   ...
   assert.ok((await chat.evaluate("document.querySelector('.hps-messages').textContent")).includes('민트플라워'));await shot('model-switched-conversation');
  }
