...
try{
 ...
 if(models){await verifyModels({app,window,findContext,cases:modelCases,out,live,upstream,gatewayCalls,layoutOnly:process.env.HPS_LESSON_MODEL_LAYOUT_ONLY==='1',setZoom:factor=>{const path=userDir+'/User/settings.json';writeFileSync(path,JSON.stringify({...JSON.parse(readFileSync(path,'utf8')),'window.zoomLevel':Math.log(factor)/Math.log(1.2)}));}});}
 else if(identity){
