...
try{
 ...
 async function findContext(selector){for(const t of await targets()){const c=await connect(t);for(const contextId of c.contexts){const evaluate=async expression=>(await c.send('Runtime.evaluate',{expression,contextId,returnByValue:true,awaitPromise:true})).result?.value;if(await evaluate('document.visibilityState==="visible" && !!document.querySelector('+JSON.stringify(selector)+')'))return {...c,contextId,evaluate};}}return null;}
 if(models){await verifyModels({app,window,findContext,cases:modelCases,out,live,upstream,gatewayCalls,setZoom:factor=>{const path=userDir+'/User/settings.json';writeFileSync(path,JSON.stringify({...JSON.parse(readFileSync(path,'utf8')),'window.zoomLevel':Math.log(factor)/Math.log(1.2)}));}});}
 else if(identity){
