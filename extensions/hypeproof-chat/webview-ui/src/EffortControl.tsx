...
const labels: Record<CourseEffort,string> = {low:'빠르게',medium:'균형',high:'깊게'};
export function EffortControl({config,post}:{config:ChatConfig;post:(m:WebviewMessage)=>void}) {
  ...
  return <div className="hps-effort">
    {policy && <label>처리 수준 <select aria-label="처리 수준" value={policy.value} disabled={policy.allowed.length===1}
    ...
    {(policy||result) && <details><summary>처리 설정 확인</summary>
