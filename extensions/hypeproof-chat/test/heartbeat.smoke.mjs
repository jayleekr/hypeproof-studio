...
// ─── 양성 — 채팅과 무관하게 계속 보낸다. 그게 이 기능의 존재 이유다 ─────────
{
  ...
  assert.deepEqual(h.sent[0], { type: "heartbeat", state: "active", idle_ms: 5_000 });

  // 오래 조용하면 active → idle. 이게 '읽는 중'과 '죽었다'를 가르는 축이다.
  h.setIdle(HEARTBEAT_IDLE_AFTER_MS + 1);
  await h.pinger.tick();
  assert.equal(h.sent[3].state, "idle");
  console.log("✓ 양성 — 채팅 없이도 핑 · idle 전이");
}
...
// ─── idle_ms 방어 — 시계 역행/NaN 이 음수 페이로드가 되지 않는다 ─────────────
{
  ...
  console.log("✓ idle_ms — 시계 역행/NaN 방어");
}
