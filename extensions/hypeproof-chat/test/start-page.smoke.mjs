...
const chat={openInEditor:async()=>{editorStarts++;},ensureProfile:async()=>stored?.startsWith('valid')?profile:null,profileFailure:()=>null,invalidateProfile(){},refreshConfig(){},hasActiveStream:()=>busy,setConnectionChanging(v){this.changing=v;},coachDisplayName:p=>p?.ux?.coach?.fallback_name||'코치',coachNameIsChosen:p=>p?.ux?.coach?.naming_mode==='fixed'};
...
assert.deepEqual(commands.slice(before),[]);assert.equal(editorStarts,1);assert.equal(page.panel,undefined,'entry canvas handed to chat');
assert.equal(page.started,true);assert.equal(page.busy,false);
page.panel={webview:{postMessage:async msg=>states.push(msg)}};
