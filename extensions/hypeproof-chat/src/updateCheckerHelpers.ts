...
export interface WindowsUpdateWrapperInput {
  ...
  // How long to wait for the app to exit before installing anyway (safety net).
  maxWaitMs?: number;
}
...
export function renderWindowsUpdateWrapper(input: WindowsUpdateWrapperInput): string {
  ...
  return [
    ...
    "while ((Get-Process -Name $name) -and ((Get-Date) -lt $deadline)) { Start-Sleep -Milliseconds 400 }",
