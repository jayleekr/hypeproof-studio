...
export function isUpdateVersion(value: unknown): value is string {
  return typeof value === "string" && /^v?(0|[1-9]\d*)\.(0|[1-9]\d*)\.(0|[1-9]\d*)(?:-[0-9A-Za-z.-]+)?(?:\+[0-9A-Za-z.-]+)?$/.test(value) &&
    value.replace(/^v/, "").split(/[+-]/)[0]!.split(".").every(n => Number.isSafeInteger(Number(n)));
}

export function compareVersions(a: string, b: string): number {
  const parsed = (s: string) => {
    if (!isUpdateVersion(s)) return null;
...
export interface WindowsUpdateWrapperInput {
  // App executable base name WITHOUT ".exe" (Get-Process -Name form), e.g.
  // "HypeProof Studio". The wrapper waits for every process of this name to exit.
  appProcessName: string;
  // Absolute path to the downloaded Inno installer .exe.
  installerPath: string;
  // Args passed through to the installer (e.g. /silent /mergetasks=runcode /LOG=…).
  installerArgs: string[];
  // How long to wait for the app to exit before cancelling safely.
  maxWaitMs?: number;
}

/**
 * Render the PowerShell wrapper that runs the Windows updater installer AFTER
 * the app has fully exited.
 *
 * Why a wrapper: the extension host process dies together with the app when we
 * quit it, so we can't quit-then-install inline. Instead we spawn this detached
 * wrapper first, THEN quit. The wrapper polls until no `appProcessName` process
 * remains (so the old single-instance lock is released), then launches the
 * installer. Without this, the installer's post-install relaunch
 * (`/mergetasks=runcode`) can fire while the quitting old instance still holds
 * the single-instance lock — the new process then hands off to the dying old
 * one and exits, leaving nothing running (the "updated but didn't relaunch"
 * bug). Mirrors the macOS installer's pgrep-wait discipline.
 *
 * ENCODING (한글 경로 결함) — the returned string STARTS WITH A UTF-8 BOM, and
 * the caller must write it verbatim (`fs.writeFileSync(p, render(...))`, default
 * utf8). This is not cosmetic. Windows PowerShell 5.1 — what `powershell.exe`
 * resolves to on every venue machine — decodes a BOM-less `-File` script as the
 * **ANSI codepage** (CP949 in Korea), not UTF-8. The installer path we bake in
 * lives under `%TEMP%` = `C:\Users\<계정명>\AppData\Local\Temp\…`, so on a
 * Korean-username machine (the common case for our cohorts) the path turned to
 * mojibake, `Start-Process` found nothing, and — because runUpdateWindows quits
 * the app right after spawning this wrapper — Studio shut down and never came
 * back updated. Measured on Windows 11 / PS 5.1.26100 with `C:\Users\신제형`:
 *
 *   BOM-less  → Test-Path 'C:\Users\신제형\…\Setup.exe'  = False
 *   UTF-8 BOM → Test-Path 'C:\Users\신제형\…\Setup.exe'  = True
 *
 * PS 5.1 and PS 7 both honor a UTF-8 BOM, so this is safe on either.
 */
export const WINDOWS_UPDATE_WRAPPER_BOM = "\uFEFF";

export function renderWindowsUpdateWrapper(input: WindowsUpdateWrapperInput): string {
  const { appProcessName, installerPath, installerArgs, maxWaitMs = 60000 } = input;
  // PowerShell single-quote escaping: double any embedded single quote.
  const ps = (s: string): string => "'" + String(s).replace(/'/g, "''") + "'";
  const argList = installerArgs.map(ps).join(", ");
  return [
    WINDOWS_UPDATE_WRAPPER_BOM +
      "# Auto-generated by HypeProof Studio win32 updater (#447). Do not hand-edit.",
    "# Waits for the app to fully exit, THEN runs the installer, so the",
    "# post-install relaunch does not race the old single-instance lock.",
    "$ErrorActionPreference = 'Stop'",
    `$name = ${ps(appProcessName)}`,
    `$deadline = (Get-Date).AddMilliseconds(${Math.max(0, Math.floor(maxWaitMs))})`,
    "while ((Get-Process -Name $name -ErrorAction SilentlyContinue) -and ((Get-Date) -lt $deadline)) { Start-Sleep -Milliseconds 400 }",
    "if (Get-Process -Name $name -ErrorAction SilentlyContinue) { throw 'Studio is still running; update cancelled without installing.' }",
    "# Grace for the OS to release the single-instance lock after the last exit.",
    "Start-Sleep -Milliseconds 1200",
    `Start-Process -FilePath ${ps(installerPath)} -ArgumentList ${argList}`,
    "",
  ].join("\r\n");
}
...
export function parseLatestRelease(
  ...
  if (!Array.isArray(release.assets)) return empty;
  const asset = pickAsset(release.assets.filter(a => a && typeof a.name === "string"));
 ...
 *   - Stages and backs up beside OLD so each rename is on the same volume.
 *   - Never deletes OLD if backup fails; restores it if installation fails.
 *   - Strips quarantine AFTER swap (a fresh xattr -dr).
 *   - Refuses to replace an app that is still running (including cancelled quit).
...
PROCESS_PATTERN=${shJsonString("^" + oldAppPath.replace(/[.*+?^${}()|[\]\\]/g, "\\$&") + "/Contents/MacOS/")}
...
# The extension requests a normal quit, preserving the unsaved-work prompt.
# Cancelling that prompt must cancel installation, not overwrite a running app.
for i in {1..30}; do
...
pgrep -f "$PROCESS_PATTERN" >/dev/null 2>&1 && abort "Studio is still running; update cancelled"

STAGE=$(mktemp -d "$(dirname "$OLD_APP")/.hps-update.XXXXXX") || abort "cannot stage beside app; check folder permissions"
BACKUP_PATH="$STAGE/previous.app"
restore_on_exit() {
  status=$?
  if [[ $status -ne 0 && -d "$BACKUP_PATH" && ! -e "$OLD_APP" ]]; then
    mv "$BACKUP_PATH" "$OLD_APP" || { echo "[RECOVERY] previous app retained at $BACKUP_PATH"; exit 1; }
    open "$OLD_APP" || true
  fi
  # Preserve any backup for manual recovery, including a failed relaunch.
  if [[ ! -d "$BACKUP_PATH" ]]; then rm -rf "$STAGE"; fi
}
trap restore_on_exit EXIT
ditto "$NEW_APP" "$STAGE/next.app" || abort "could not stage new app; old app unchanged"
mv "$OLD_APP" "$BACKUP_PATH" || abort "could not back up old app; refusing to delete it"
echo "[ok] backed up old → $BACKUP_PATH"
mv "$STAGE/next.app" "$OLD_APP" || abort "could not install new app; restoring previous app"
...
# Reopen
open "$OLD_APP" || abort "could not launch updated app; previous app retained at $BACKUP_PATH"
