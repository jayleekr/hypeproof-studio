...
| # | 무엇 | 어떻게 막았나 |
|---|---|---|
| **F-1** | `/v1/profile`·`/observations/context` 가 `/1` 하드코딩 → P1 화면 도달 불가 | `Profile.observation.format` 신설(미선언 = `/1`, 기존 코호트 불변), 두 라우트가 **같은 함수** `servedObservationFormat()` 을 부른다. 신규 `worker/test/observation-format-served.test.mjs` 9검사 — 라우트 응답을 직접 읽는다 |
| **F-1b** | 코호트 중 `/2` 를 선언한 곳이 없으면 여전히 죽어 있다 | **canary-sdk-contract** 가 선언한다(로스터 0명, 사람 없음). 검사가 "적어도 한 코호트가 `/2` 를 선언한다" 를 단언한다 |
