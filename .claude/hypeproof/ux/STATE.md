...
> ### 2026-09-20 마지막 — **요구를 단단하게 만들었고, 다음은 ADR 0010 구현이다**
>
> 이 세션은 구현(P0·P1) → 평가 2회 → 수리 2회 → **요구 전수 점검** 순서로 갔다.
> 마지막 두 시간이 가장 값이 컸다. 구현을 더 쌓는 대신 **요구가 맞는지 먼저 봤다.**
>
> **마스터 프롬프트가 v3 으로 바뀌었다** (`MASTER_PROMPT.md`). v1 은 오늘 뚫린 구멍
> 여덟 개 중 여섯 개를 구조적으로 막을 수 없었고, v2 초안은 **자기모순 세 개**로 폐기했다.
> v3 은 완료의 정의를 **여섯 게이트**로 바꿨다 — 특히 **G4 도달 가능성**(이 기능이
> 실제로 켜지는가)과 **G6 CI 가 끝날 때까지 본다**가 신설이다.
>
> **요구 점검 결과**: `REQUIREMENTS-HARDENING-2026-09-20.md`.
> 에이전트 83개, 81건 보고 → **32건 확인, 45건은 반박으로 기각.**
> 가장 큰 것: **요구 60행 중 42행이 엉뚱한 검사를 가리키고 있었다.** 고쳤고,
> `worker/test/sx-traceability.test.mjs` 가 양방향으로 세서 다시 생기지 못하게 한다.
>
> **Jay 가 결정한 7건은 이미 문서에 반영됐다. 다시 묻지 않는다** (아래 "결정된 것").



**2026-09-20 — P0·P1 완료(각각 평가 1회·수정 1회). 머지 안 함. PR 넷.**

> **P1 평가: FAIL 62/100, CRITICAL 0/10.** 판정서 `judge-P1-2026-09-20.md`.
> 평가자가 **P1 의 화면이 어떤 빌드에서도 렌더되지 않는다**는 것을 찾았다 —
> `chat.ts` 가 `observation.format` 을 `/1` 로 하드코딩하고 있어서 `/2` 좌석이
> 존재할 수 없었다. 게이트도 서랍도 전부 만들어 놓고 **도달 불가**였다.
> 그 자리에서 수리했고(아래 "평가 뒤 수리"), 수리 자체도 대조군으로 막았다.
...
- `feat/sx-p1c-before-after` (#1177 위에 쌓임) → **PR #1178, P1-C + P1-D.** SX-24 만 남았다.
  머지 순서는 **#1170 → #1176 → #1177 → #1178** 이다. 각 PR 의 diff 는 바로 아래
...
## 평가 뒤 수리 (2026-09-20, P1)

판정서의 F-1~F-10 중 **여덟 건을 고쳤다.** 각 수리에 대조군을 같이 넣었고,
평가자가 심었다가 놓쳤던 결함 둘을 **다시 심어** 이제 잡히는 것을 확인했다.

| # | 무엇 | 어떻게 막았나 |
|---|---|---|
| **F-1** | `/v1/profile`·`/observations/context` 가 `/1` 하드코딩 → P1 화면 도달 불가 | **1차 수리는 실패했다(아래 "2차 수리").** 2차에서 클라이언트 호출부 4곳을 `observationHeaders()` 하나로 모았다. 검사 12개 |
| **F-1b** | 코호트 중 `/2` 를 선언한 곳이 없으면 여전히 죽어 있다 | **canary-sdk-contract** 가 선언한다(로스터 0명). **부작용 없음이 아니다** — 아래 "카나리 부작용" |
| **F-1c** | `/2` 가 열리면 서랍 옆에 "관찰을 지원하지 않습니다" 가 같이 뜬다 | `ChatPanel` 의 두 조건을 `=== '/1'` 에서 `isObservationFormat()` 으로 바꿨다 |
| **F-2** | `actor=ai` 기대 조건 단언이 공전(텍스트 절이 먼저 false) | 시료에 `student_text` 를 넣어 actor 절만 남겼고, `student_text` 칸이 **없는** `test_observed` 로 양성/음성 한 쌍을 더했다. 평가자 결함 #14 재심기 → **CAUGHT** |
| **F-4** | `/1` "byte for byte" 주장이 증거보다 넓고, `isHumanEvidence` actor 가드 무검사 | 주석을 "그 코퍼스에 대해서" 로 좁히고 규칙이 **움직였다**는 것을 적었다. actor 가드 단언 5줄 추가. 평가자 결함 #15 재심기 → **CAUGHT** |
| **C5** | `record()` 가 학습 kind 를 막지 않는다(내가 스스로 찾음) | `isLearningEventKind(kind)` 면 `learning_kind_needs_form` 으로 던진다. "오늘 아무도 그렇게 부르지 않는다" 는 관습이지 규칙이 아니었다. 음성 2 · 양성 3 |
| **F-6** | 작업 화면에 Primary CTA 둘, 세는 규칙 없음 | 완료 버튼을 `hp-cta-quiet` 로 내리고 `auditPrimaryCta()` 를 감사기에 넣었다(마크업을 센다). 렌더가 D 영역을 포함하지 못하므로 **소스 정적 검사**를 함께 두고 그 한계를 주석에 적었다 |
| **F-8** | 어투가 한 기능 안에서 갈린다 | P1 이 만든 문장을 해요체로 통일했다(코호트가 초등 5–6) |
| **F-9** | `data-source-state`·amber 는 원값, 라벨만 정규화 | `normalizeSourceState()` 를 한 번 부르고 셋 다 그 값을 쓴다. 모르는 값 렌더 대조군 추가 |
| **F-3** | "`Task.curriculum` 이 없다" 는 **내 보고가 틀렸다** | 개정 제안 12번을 정정했다. 상태 기계는 출하됐고 **없는 것은 호출부 하나**다 |
| **F-5** | 제안 9번이 결과를 끝까지 말하지 않는다 | "채택한 기대 조건은 게이트를 **통과한다**. SX-14 와 반대다" 를 명시했다 |

**고치지 않은 것 둘.**

- **F-7** (구현 전 대조군이 "파일이 없어서 빨강") — 이미 만든 task 에 소급 적용할
  방법이 없다. 다음 task 의 control 을 쓸 때 **기존 파일의 기존 함수**에 걸리는
  단언을 하나 넣는 규칙으로 받는다.
- **F-10** (SX-T 전부 NOT RUN) — Electron e2e 가 필요하고 앱 빌드는 **사용자 승인
  사항**이다. 승인이 없었다. 아래 "사람에게 묻는 것" 에 올렸다.

**F-1 이 드러낸 사람 결정 하나.** 지금 `/2` 를 선언한 것은 카나리뿐이다.
**어느 실제 코호트가 언제 `/2` 로 가는가**는 수업 결정이지 코드 결정이 아니다.
프로필 한 줄이고 되돌리는 것도 한 줄이다.

## 2차 수리 (2026-09-20) — 1차 수리가 F-1 을 못 고쳤다

독립 검증자를 붙였고 **내 수리가 실패했다고 판정했다.** 세 가지가 나왔다.

### 1. F-1 은 여전히 살아 있었다 — 마지막 고리에서

1차 수리는 서버 두 라우트가 **같은 함수**로 협상하게 만들고 주석에 이렇게 적었다:

> "Same negotiation, same function, so the two answers cannot drift."

**틀렸다.** 두 라우트는 안 갈라졌지만 **두 호출부가 갈라졌다**:

| 호출부 | 보낸 헤더 | 받은 포맷 |
|---|---|---|
| `proxyClient.ts` → `/v1/profile` | `x-hps-observation-format: /2` | `/2` ✅ |
| `chatPanelProvider.ts` → `/observations/context` | `authorization` **만** | `/1` |

레코더는 **컨텍스트 응답**으로 만들어진다. 그래서 `batch.format === '/1'` →
`currentLearningRecorder()` 가 계속 `null` → **서랍은 그대로 죽어 있었다.**

그런데 **내가 쓴 9개 검사는 전부 초록이었다.** 검사가 `servedFor()` 로 **같은 헤더를
양쪽에 보내서** 재고 있었기 때문이다. 실제 클라이언트는 그렇게 부르지 않는다.
**계측기가 제품이 아니라 자기 자신을 재고 있었다.**

고친 것: 클라이언트 헤더를 `observationHeaders()` **한 곳**에서만 만든다.
확장 소스를 정적으로 훑어 "관측 라우트를 부르면서 헤더를 손으로 만드는 자리"를
0건으로 단언한다. 그 단언이 **곧바로 네 번째 호출부**(`/observations/assess`)를 찾아냈다 —
검증자도 못 본 자리다.

### 2. 1차 수리가 **회귀**를 만들었다 (사용자에게 보이는 것)

클라이언트 헤더를 `/1`→`/2` 로 올려 놓고 `chat.ts` 의 배너 조건은
`!== "hps-observation/1"` 로 남겨 뒀다. 결과: **관측이 켜진 모든 좌석**이 인사말 뒤에
*"이 앱 버전은 작업 관찰 화면을 지원하지 않습니다… Studio를 업데이트해 주세요"* 를 받는다.
`studio-native-trial` 실측 88→161 바이트.

**이것이 세 번째 `/1` 리터럴이었다.** 1차 수리는 `ChatPanel` 의 두 개를 고치면서
이것을 세지 않았다 — 커밋 본문에서 바로 그 실수(규칙 1b, "생성 지점을 센다")를
사과해 놓고 같은 실수를 저질렀다. 이제 `isObservationFormat()` 을 쓴다.

### 3. 카나리 부작용 — "한 줄이고 되돌리기도 한 줄" 은 좁은 진실이었다

카나리에 관측을 켜면 `/v1/observations/*` 가 404 대신 응답한다.
`scripts/run-sdk-contract.sh` 가 **프로덕션에 대고** 이 코호트를 돌린다.
그 스크립트는 `/v1/profile` 200 과 SDK 요청 모양만 보므로 **깨지지는 않는다**(확인했다).
그래도 "부작용 없음" 은 내가 재보지 않고 쓴 말이었다.

### 대조군

검증자가 찾은 결함 둘을 포함해 **6개를 심어 6개를 잡는다.** 처음엔 5/6 이었고
못 잡은 하나가 바로 "컨텍스트 호출부가 헤더를 빠뜨린다" — **워커 검사로는 잡을 수 없다**
(워커는 확장 호출부를 돌리지 않는다). 그래서 확장 쪽에 정적 검사를 넣었다.

### 남은 진실

`/2` 를 선언한 것은 여전히 카나리뿐이다. **이 기능을 보는 학생은 오늘 0명이다.**
...
| P1 | P1-PR | | | **부분 범위 PR 셋** — #1176(P1-A), #1177(P1-B), #1178(P1-C·D) | 3 |
...
## 결정된 것 (2026-09-20 Jay) — 다시 묻지 않는다

| 결정 | 어디에 반영됐나 |
|---|---|
| 회고를 건너뛴 **사실을 남긴다** | `reflection_skipped` 이벤트. phase 값은 안 늘린다 (SX-55·SX-26) |
| 선택 이유는 **필수**다 | SX-23 필수로 · SX-45 의 "이유 미기록" 폐기 · SX-T28 을 반대로 뒤집음 |
| 막힌 학생에게는 **단계적으로 더 도와준다** | SX-07 상한을 "막히지 않은 상태" 로 좁힘. 막히면 SX-08 사다리가 우선 |
| 첫 화면에 **세어 본 사실은 허용** | SX-30 부정을 점수·등급·퍼센트·진행률로 좁힘 |
| 파일은 6단계, **화면에만 3개씩** | SX-02 의 "4개 이상 파일 거부" 를 화면 규칙으로 옮김 |
| 서랍은 **그 단계가 요구하면 한 번** 열린다 | SX-17 부정을 "학생이 닫은 뒤 다시 열리면 실패" 로 |
| 관측은 **모든 반에 켠다. 단 플래그를 먼저 쪼갠다** | **ADR 0010** (`docs/adr/0010-observation-capability-split.md`) |
| 아이 반은 **`assess` 를 끈다** | 같은 ADR. 기록·서랍·게이트는 켜되 Anthropic 전송만 뺀다 |
...
### H-P1 · 어느 실제 코호트가 `hps-observation/2` 로 가는가 (결정 필요)

P1 의 화면(완료 게이트·Evidence drawer)은 좌석이 `/2` 를 받아야만 존재한다. 지금
`/2` 를 선언한 것은 **카나리뿐**이고, 카나리에는 사람이 없다. 즉 **오늘 이 기능을
보는 학생은 0명**이다.

- 켜는 법: 그 코호트 프로필에 `observation: { enabled: true, format: 'hps-observation/2' }`
- 끄는 법: `format` 한 줄 삭제
- 안전장치: 구버전 앱에는 서비스가 **자동으로 `/1` 로 낮춰** 준다
  (`servedObservationFormat`). 업데이트 안 한 학생이 깨지지 않는다.
  **반대 방향도 적어 둔다(검증자 지적):** 1차 수리에서는 *업데이트한* 학생이
  `/1` 코호트에서 "업데이트하세요" 배너를 받는 회귀가 있었다. 2차에서 고쳤고
  양쪽 방향 모두 검사가 있다.
- 내가 정하지 않은 이유: 어느 주차에 무엇을 보여 줄지는 **수업 결정**이고,
  마스터 프롬프트가 기존 코호트 프로필 변경을 금지한다.

### H-P1b · SX-T 실기 실행에 필요한 앱 빌드 승인

루브릭 A 는 SX-T 행이 실제로 돌아 초록이어야 점수를 준다. 지금 전 행이 `NOT RUN` 이고,
이 세션의 어떤 초록도 그것을 대신하지 않는다. 평가자가 세 행(SX-T04·T23·T26)은
`HPS_APP_PATH` 주입 경로로 **앱 빌드 없이** 갈 수 있다고 적었다 — 그 경로부터 볼지,
아니면 `bash build.sh` 를 승인할지는 사람 결정이다(빌드는 1~2시간·10~20GB).
...
9. **P1-A 가 찾은 설계 문서 모순 넷 — 문자 그대로면 쓸 수 없는 것이 나온다.**
   ...
   - **`criterion_set` 의 `student_text` 필수와 SX-14 가 충돌한다.** SX-14 는 코치가 제안한
     ...
     **결과까지 적는다(2026-09-20 보강):** 그래서 학생이 폼에서 제출한 채택 기대 조건은
     `actor=user` + `adopted_from` 으로 저장되고 **완료 게이트를 통과한다.** SX-14 본문이
     "통과하지 못한다" 고 적은 것과 **반대다.** 구현은 설계 §관측 이벤트와 필드 규칙 4 를
     따랐고 `sx-evidence-gate.smoke.mjs` 가 그 통과를 명시적으로 단언한다.
     저장 가능성만 적고 이 결과를 안 적으면, 사람이 이 제안만 읽고는 어떤 결정이
     내려졌는지 알 수 없다. **정본은 사람이 고른다.**
...
10. **P1-A 가 남긴 부채 넷 (범위 밖이거나 결정 대기).**
   - ~~`worker/src/routes/observations.ts` 의 `POST /observations/validate` 가 `/2` 배치를 받아
     놓고 응답에 `format: OBSERVATION_FORMAT` 을 하드코딩한다~~ → **2026-09-20 고쳤다.**
     그리고 이 항목을 적으면서 **더 큰 것을 세지 않았다**: `chat.ts` 가 `/v1/profile` 의
     `observation.format` 을 똑같이 하드코딩하고 있었고, 그것이 P1 의 화면 전체를
     도달 불가로 만들고 있었다(평가자 F-1). `/1` 하드코딩 **지점을 전부 세었어야**
     했다 — verification.md 규칙 1b 의 "생성 지점을 센다" 가 이 경우에도 적용된다.
...
12. **P1-B 가 남긴 빚 하나 (범위였는데 못 끝낸 것).** ← **2026-09-20 정정됨**

   처음에 "`Task.curriculum` 영속화와 phase 전이가 없다" 고 적었다. **틀렸다.**
   평가자가 세어서 잡았고 나도 다시 세어 확인했다:

   - **있는 것**: `CurriculumPhase`·`TaskCurriculum`·`updateCurriculum()`·전이표
     (한 칸씩 앞으로만)·뒤로 가기 거절·SX-55 부정 조건(`submitted` 없이 `reflected`)
     거절이 `local-record.ts:122~210` 에 전부 출하됐고 `learning-events.test.mjs` 가
     단언한다.
   - **없는 것**: **호출부 하나.** `grep -rn "updateCurriculum" extensions/ worker/src`
     가 정의 한 줄만 돌려준다. `LearningStatePayload.phase` 는 `?? "working"` 으로
     고정돼 내려간다.

   틀린 방향이 안전한 쪽(과소)이라 위험하지는 않았지만, **다음 세션이 이미 있는 것을
   다시 만들 뻔했다.** 남은 일은 "상태 기계를 만든다" 가 아니라 **"`submitTask` 핸들러가
   `updateCurriculum` 을 부르고 그 결과를 영속화한다"** 하나다.

   화면에서는 여전히 "완료가 열린다" 까지만 참이고 **"완료된다" 는 아직 참이 아니다.**
...
**ADR 0010 구현 — `observation.enabled` 를 쪼갠다.**
전문: `docs/adr/0010-observation-capability-split.md`. **읽고 시작한다.**

### 왜 이것이 먼저인가

P1 이 만든 화면(완료 게이트·Evidence drawer)은 좌석이 관측을 받아야 존재한다.
지금 그것을 선언한 것은 **카나리뿐**이라 **이 기능을 보는 학생이 0명**이다.
그리고 그냥 전부 켜면 안 된다 — 조사에서 이 플래그 하나가 **네 가지를 겸하고** 있는 것이
드러났다(실측):

| 딸려 움직이는 것 | 측정 |
|---|---|
| `/v1/profile` 세션 게이트 | 9개 프로필 중 **7개가 200 → 403** |
| 채팅 기록 저장 키 | 기존 학생의 **지난 대화가 사라진다** |
| 개인 체험 좌석 발급 자격 | **아이 반에도** 체험 토큰이 발급 가능해진다 |
| 평가 모델 라우팅 | GPT·GLM 반은 **502**, 버튼이 항상 실패 |

### 순서 (ADR §마이그레이션)

1. `record`·`assess` 를 **더한다**. `enabled` 는 당분간 남긴다
2. **세션 게이트**와 **체험 좌석 발급**을 각자의 조건으로 뗀다
3. 평가 라우트가 공급자에 맞는 모델을 고르거나 **이름 있는 거절**을 한다(502 로 뭉개지 않는다)
4. `record` 기본값을 켠다
5. `enabled` 제거는 별도 커밋

**각 단계마다 9개 프로필 전부의 `/v1/profile` 응답을 찍어 비교한다.**
라우트 응답이 계약이다 — 헬퍼 단위 테스트는 이것을 못 잡는다(F-1 이 그렇게 지나갔다).

### 이미 있는 것 (다시 만들지 않는다)

- `servedObservationFormat()` — 포맷 협상. 구버전 앱에 자동으로 낮춰 준다
- `observationHeaders()` — 클라이언트 헤더 한 곳. 호출부 4곳이 전부 여기를 쓴다
- `worker/test/observation-format-served.test.mjs` — 12검사, **라우트 응답을 직접 읽는다**
- `Task.curriculum`·`updateCurriculum()`·전이표 — **전부 출하됐다. 없는 것은 호출부 하나다**

### 그 다음

- **SX-24** 팀 기여 — `/2` 키 allowlist 에 칸을 연다. #1176 머지 뒤
- **P2** 경계 회고 (`ux-dag.yaml` P2-A·P2-B)
- **검증 문서 층·명령 색인 드리프트** — 약 30곳. 행별 열이 정본

### 넘어온 빚

- **실기 증거가 여전히 0이다.** MASTER_PROMPT v3 §2 G5 에 싼 경로를 적어 뒀다 —
  `scripts/test-native-trial-laptop.sh` 를 쓰고, **화면이 잠겨 있을 때만** 띄운다
- `Task.curriculum` **호출부**가 없다. "완료가 열린다" 까지만 참이고 "완료된다" 는 아직이다
