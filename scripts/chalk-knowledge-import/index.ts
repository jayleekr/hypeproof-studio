...
function buildSql(
  ...
  const lines: string[] = [
    ...
    `-- digest: ${digest}`,
    `-- atomicity: local wrangler d1 execute --file rolled back fully on a mid-file failure (2026-09-25).`,
    `-- Remote not verified — after J3 load run:`,
    `--   SELECT doc_count FROM chalk_knowledge_versions WHERE version=N;`,
    `--   SELECT COUNT(*) FROM chalk_knowledge_docs WHERE version=N;`,
    `--   on mismatch: DELETE FROM chalk_knowledge_docs WHERE version=N;`,
    `--              DELETE FROM chalk_knowledge_versions WHERE version=N;`,
    "",
