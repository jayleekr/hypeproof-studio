# Contributors

HypeProof Studio is built by this team. Thank you.

| Name | GitHub | Role |
|---|---|---|
| Jae Won (Jay) Lee | [@jayleekr](https://github.com/jayleekr) | Maintainer |
| Bongho Tae | [@xoqhdgh1002](https://github.com/xoqhdgh1002) | Contributor |
| Jinyong Shin | [@JinyongShin](https://github.com/JinyongShin) | Contributor |
| Shin (제형) | [@JeHyeong2](https://github.com/JeHyeong2) | Contributor |
| Jkim | [@ico1036](https://github.com/ico1036) | Contributor |
| Taejin Kang (TJ) | [@TJ-kr](https://github.com/TJ-kr) | Contributor |

<!--
Keep this list in sync when collaborators are added/removed.
Source of truth for access: `gh api /repos/jayleekr/hypeproof-studio/collaborators`.
Add a row when an invite is sent; names are GitHub profile display names.
-->
