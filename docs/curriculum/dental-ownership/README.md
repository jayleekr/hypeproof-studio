...
초안 저장 API는 PR #706에 의존하며, 이 자동화의 PR은 그 브랜치를 대상으로 한다.
...
같은 코호트에서 다른 강사가 복제하려면 새로운 course_id가 필요하다. 기존 초안의 실제 편집은 revision을 사용하는 authoring API/후속 Chalk UI의 책임이다.
...
실제 수강생 장애 티켓으로 간주하지 않는다. 현재는 파일 기반 목록이며 Chalk 내 접수·처리 상태 UI는 후속 기능이다.
...
- 학생 Studio UI·실제 강사 강의 진행·학생 기기 배포 리허설: 미실행.
