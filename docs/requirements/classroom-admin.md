...
| 대상별 결과와 재선택 (2026-09-21 보완) | 선택 좌석마다 **두 원장**(회수 item + 기기 요청)에서 한 상태를 정한다: 검증됨 / 전송·검증 대기(바이트 도착 중, 기기가 전송 완료 보고) / 기기 응답 대기(서버 접수·배정은 ‘기기 수신 확인 전’, 기기 receipt 뒤에만 ‘수신’) / 실패·미도착(failed·rejected·expired·cancelled·unsupported·연결 없음·불완전) / 결과 미확인(`outcome_unknown` — 성공으로도 실패로도 세지 않는다) / 제외(동의 없음·철회) / 격리. `requested`만으로 ‘진행 중’이라 하지 않는다. 실패·미확인 대상은 알려지는 즉시 다시 선택할 수 있고, 다른 대상이 진행 중이어도 막지 않는다. 대기·전송 중·검증됨·제외·격리는 재선택에 넣지 않는다. 재선택 뒤 요청은 **새 회수 요청**이며, 앞선 요청으로 이미 시작된 전송은 `upload_until`까지 도착하면 그대로 검증된다고 화면이 말한다 | AT-37 |
