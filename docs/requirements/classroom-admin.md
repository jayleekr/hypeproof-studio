...
| 기기 보관 | 쓰던 질문은 (코호트·프로필·학생·회차)에, 미리 본 요청은 거기에 연결 grant까지 묶어 저장한다. 다른 학생·수업·연결에서는 보이지도 보내지지도 않는다. 보내기 직전과 응답 도착 뒤에 다시 대조한다. 동의 후 보내기 전에 ‘보내는 중’을 먼저 기록하고, 응답이 사라지면 `unknown` — 같은 id·같은 내용으로만 다시 확인한다(중복 저장 0). 24시간 손대지 않은 다른 학생의 초안은 기기에서 지운다. 서명된 종료 시각이 없는(이전 형식) 요청이나 그 시각이 지난 요청은 보내지 않고 다시 미리 보게 한다. |
...
| 여러 창의 기기 저장 (2026-09-22 U4) | 기기 저장(`globalState`)은 앱의 모든 창과 공유 PC의 모든 학습자가 함께 쓴다. adapter는 await 전에 읽은 사본을 쓰지 않는다 — 변경은 **그 순간의 저장본에 같은 틱 안에서** 적용하고, 요청 상태 변경은 요청 id 비교 후에만(compare-and-set) 한다. 그래서 한 창의 미리보기가 다른 창·다른 학생의 초안·요청을 지우지 않고, 보내는 중·응답 불명 요청은 새 미리보기로 바뀌지 않으며, `sending`/`unknown`이 되고 POST가 나가는 것은 학생이 동의한 요청 id뿐이다. **남는 한계:** 같은 학습자가 두 창에서 같은 초안을 고치면 글자는 마지막 저장이 남는다(요청·동의 내용은 영향 없음). VS Code는 확장의 globalState 전체를 한 덩어리로 쓰므로 두 창이 저장 전파 지연(수 ms) 안에 동시에 쓰면 한쪽이 남는다 — adapter가 닫을 수 없고, 그 폭을 한 틱보다 넓히지 않을 뿐이다. |
