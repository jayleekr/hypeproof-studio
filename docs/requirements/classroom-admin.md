...
학생별 일시정지(새 table·gate 변경) · 이미 승인된 turn의 후속 요청 유예 · 표본 간격보다 짧은 재개 실행의 관측(해제에 묶인 1회 turn 보고) · SDK 경로 실패의 원인 분류(실기: 공급자 5xx가 `unknown`으로 보고돼 ‘공통 장애’ 안내·묶음이 뜨지 않음 — 판정은 `문제 남음`까지) · 미리보기 상태를 앱이 보드에 보고하는 신호 · 재발급 토큰의 시작 화면 입력을 실제 창에서 확인 · 실제 창에서의 미리보기 서버 재시작(새 포트) · 세 흐름 공통 결과 화면(AT-41)의 나머지 · U1b 종류별 회수. 사람 결정: 위 판정 어휘의 운영 승인(INT-CO 범위), `resolved`를 운영 지표로 쓸지.
