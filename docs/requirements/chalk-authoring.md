...
저작은 Chalk(웹)에 있고 **리허설은 Studio(앱)에서 돈다** — 강사의 로그인·로컬 도구가 성공을 대신하지 않으므로
실제 도구 실행과 앱/SDK 증거가 필요한 쪽을 따른다. 두 앱 사이는 ARC-03 의 교환권 링크로 잇는다.
...
| BASE-02 | Chalk에서 수업 설계·구성·운영을 제공하고, 리허설은 Studio에서 실행하도록 연결한다. | P0 |
...
| RUN-01 | 강사 전용 권한 없이 학생 조건으로 **Studio에서** 리허설한다. 학생 조건은 권한과 환경을 뜻하며, 리허설은 수업을 열기 전에 하는 일이므로 **열린 세션을 요구하지 않는다**. | P0 |
...
| ARC-03 | 수업을 가리키는 신원은 `(cohort_id, course_id, version)` 과 내용 해시 `sha256` 넷이다. 참여 코드는 신원이 아니라 한 자리의 자격증명이므로 수업 식별자로 쓰지 않는다. Chalk에서 Studio로 리허설을 여는 링크는 **일회용 교환권 하나만** 나르고 좌표와 자격증명을 URL에 노출하지 않으며, 앱은 교환권을 Service에 제출해 학생 조건 자격을 받는다. 교환권은 한 번만 통과한다. | P0 |
...
기본 예제 선택 → 초안 작성 → 준비 점검 → (Studio에서) 학생 조건 리허설 → 수업 버전 확정 →
