...
preview recovery checks the page the learner had open (a 404 is a named fault) and re-points only the learner's own
loopback preview tab after a restart. Pause still refuses only NEW model requests: a request already streaming is not
