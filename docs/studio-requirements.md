...
| REQ-G15 | 리허설을 통과하지 않은 버전은 **참여 코드를 받지 못한다** (#1187) | 상위 요구사항은 **RUN-01**·**VER-02** 이고 **새 요구사항을 만들지 않았다**. 배경: #1152 가 리허설 상태를 **읽는** 경로를 만들었지만 그 값은 응답에 실리기만 했고, 발급을 막는 조건은 넷이었다(강사 스코프 · 확정된 버전 · 열린 세션 · 로스터 멤버십). 그래서 제품이 하던 말이 "이 강의는 리허설을 안 했습니다. 그런데 학생을 들여보내세요" 였다 — 경고하고 통과시키는 것은 점검표이고 관문은 막는 것이다. 계약: ① `POST {root}/versions/:version/participants` 의 **다섯 번째 거부**로 붙고 앞의 넷은 바뀌지 않는다; ② 상태는 여기서 다시 계산하지 않고 `readRehearsal()` 이 저장된 증거에서 읽은 것을 쓴다 — 판정이 두 곳이면 어긋난다; ③ **`passed` 가 아닌 모든 값**(`not_run`·`failed`·모르는 값)을 막는다. 새 상태가 조용히 통과하지 않는다; ④ **`not_run` 과 `failed` 는 다른 말을 한다** (`reason`: `rehearsal_not_run` / `rehearsal_failed` / `rehearsal_not_passed`) — 다음 행동이 다르기 때문이다. 안 돌렸으면 돌리면 되고, 실패했으면 확정 버전이 불변이므로 **수업을 고쳐 새 버전**을 확정해야 한다; ⑤ **VER-02 는 무효화 코드 없이 성립한다** — 내용이 바뀌면 새 버전이고 새 버전에는 증거 행이 없으므로 자동으로 막힌다; ⑥ **우회 스위치를 두지 않는다.** 급할 때 반드시 눌리고 그때부터 관문은 점검표가 된다. 강사가 리허설을 **하러 가는 길**(REQ-G14 의 교환권 경로)은 이 관문 **뒤가 아니라 옆**에 있어서, 막혀도 빠져나올 수 있다; ⑦ **실패 방향은 닫히는 쪽** — `readRehearsal()` 이 저장소를 못 읽으면 `not_run` 으로 떨어지고 발급이 멈춘다. **이 행이 정하지 않는 것**: 증거를 **쓰는** 경로(#1186) — 무엇이 "리허설이 끝났다" 인지와 누가 판정하는지는 그쪽의 몫이다 | U (`worker/test/authoring-rehearsal-gate.test.mjs` — `not_run`·`failed`·모르는 상태 거부 · **`passed` 통과(양성 대조군)** · 앞선 거부 넷 보존 · 내용 변경 후 재차단 · **교환권 경로 면제(대조군: 같은 버전의 참여 코드는 막혀 있다)** · 증거 테이블 부재 시 닫힘) + I (`worker/test/authoring-d1.test.mjs` — 실제 workerd/D1 에서 같은 거부·통과) |
