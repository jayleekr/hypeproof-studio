...
| REQ-STUDIO-REHEARSAL | A learner code issued for an instructor's rehearsal (`rehearsal` claim, Service record) shows a rehearsal line in the rail; "리허설 결과 보내기" sends what the panel actually DREW per step plus the App identity. The Service judges it once against the candidate and its own records of requests made with that code; sending before any request is refused as "not executed", not judged. | Same as above; contract in [chalk-authoring G2](requirements/chalk-authoring.md#g2-curriculum-runtime-20260922). |
