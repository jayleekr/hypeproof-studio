...
| REQ-STUDIO-CHALK-REHEARSAL | The instructor starts a rehearsal from the Chalk surface, enters under student conditions in the same app, and returns | Ticket issuance carries the issuer Bearer; the redeem call carries **no** `Authorization` and the ticket only in the body (ARC-02); after the switch the chat/tool path carries the seat token and the issuer never leaves the authoring call; ending the rehearsal restores the prior participation credential in place, with no reload. A request already in flight at the moment of the switch may still land with the prior credential — bounded to the usage view, never the conversation path. No deep link: the transition happens inside one app. Client invariant only — the Service enforces both directions (#1185 · chat-gate). | Host + App; `extensions/hypeproof-chat/test/rehearsal-session.smoke.mjs`, `e2e/rehearsal-surface/run.mjs` |
