...
| REQ-M14 | 설정 기본값과 실제 런타임 구분 | `hypeproofChat.coachRuntime`의 manifest default는 `"proxy"`로 유지. 실제 선택은 Service의 `coach_runtime`과 machine 설정을 함께 읽어 어느 쪽이든 `agent-sdk`면 SDK 경로를 요청하며 미성년 조건은 proxy로 강제한다. SDK를 로드하지 못한 턴의 proxy fallback은 REQ-M7/M30을 적용. manifest 기본값만으로 모든 수업이 proxy 실행이라고 판정하지 않음 | U (`test/sdk-gateway`, `test/sdk-coach-helpers`) + E |
