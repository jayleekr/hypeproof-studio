...
| REQ-STUDIO-LESSON-MODEL | A frozen lesson may narrow which models the seat may use, within what the compiled profile already grants; the Service enforces it by narrowing the served profile so both existing clamps apply unchanged, and `/v1/profile` serves the seat's allowed set. | [ADR-0006](adr/0006-lesson-model-policy.md) · `worker/test/authoring.test.mjs` (narrowing accepted; widening refused at save and at freeze; both clamps follow; `{default}` counterexample). Narrowing-only, so no new authority: the issuer scope and the participation token are unchanged. Re-checked on every read, so a profile whose grant later shrinks fails closed as `409 lesson_unavailable`. **On `agent-sdk` seats the served set still includes `hypeproof-fast`** — `/v1/messages` admits it with no request-purpose marker, so the block reports it via `route_exception` rather than claiming a narrower policy than the route enforces (AE-25 UNMET on that route). `model.id` carries no provenance; the turn's actual model comes from its own evidence. Real-screen model display is E7's next unit and is NOT RUN. |
