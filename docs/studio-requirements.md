...
| REQ-E4 | 코치가 파일을 건드리기 **전에** 복구 지점을 뜬다 — 그리고 되돌릴 문이 있다 (#673, AE-15/WEB-05) | 2026-08-22 1회차 오후 14:47 KST, 코치가 `Edit` 로 `index.html` 을 깨뜨린 자리의 아이에게 **돌아갈 길이 없었다.** 백업이 없어서가 아니다 — `archiveCurrentWorld`(#649)는 실재하는 백업이지만 `shouldArchiveWorld` 가 "디스크 내용 ≠ 쓸 내용" 일 때만 보관하고, SDK 코치의 Edit/Write 는 `revealWrittenHtml` 이 파일을 되읽기 **전에** 이미 바꿔 놓으므로 `current === next` 가 되어 **정확히 그 시나리오에서 건너뛴다**(코드 주석이 그 사실을 적고 있었다). 거기에 복구 명령이 하나도 없어(contributes.commands 18개 중 0개), 복구는 프롬프트 산문에 — 즉 **코치가 기억해 주기를 바라는 것**에 의존했다. 계약: ① **뜨는 시점은 승인 게이트다.** 쓰기 도구는 전부 `evaluateSdkToolUse` 의 "writes ALWAYS ask" 로 `resolveActionApproval` 에 모이고 SDK 는 그 resolve 전에 도구를 실행하지 않는다 — 두 런타임(SDK·로컬 개발 실행기)이 같은 문을 쓴다. `writeFile` 과 `executeShell` 둘 다 걸린다(셸도 파일을 지운다); ② **턴당 하나.** 한 턴에 Edit 이 다섯 번이어도 되돌릴 자리는 그 턴 이전 하나다; ③ **파일 한 장이 아니라 집합이다** — 추가·수정·삭제를 모두 되돌린다. 삭제 계획이 없으면 코치가 만든 잡동사니가 남아 아이는 원래대로 왔는지 알 수 없다; ④ **디스크를 뜬다.** 앱이 기억하는 상태가 아니라 실제 폴더를 읽으므로 아이의 수동 편집과 셸이 만든 파일이 그대로 들어온다; ⑤ **되돌리기 자체가 되돌려진다** — 복구 전에 현재 상태를 먼저 뜬다(AE-15). 되돌릴 수 없는 복구는 두 번째 덮어쓰기일 뿐이다; ⑥ **복구가 복구를 먹지 않는다** — `이전 세상/` 은 보관본이 사는 곳이라 복구가 그 안을 지우지 않는다(추가 전용), 그리고 그렇게 했다고 말한다; ⑦ **저장소는 작업 폴더 밖**(확장 globalStorage)이다 — 안에 두면 셸을 든 코치가 백업을 고치고 복구 계획이 자기 자신을 되돌리려 든다; ⑧ **담지 못한 것을 숨기지 않는다** — 크기·개수 상한에 걸린 파일과 심볼릭 링크는 사유와 함께 기록되고 그 사실이 안내 문구에 나온다. 조용히 자르면 "복구했다" 가 거짓이 된다; ⑨ **바깥은 되돌아가지 않는다고 먼저 말한다** — 갤러리·배포된 주소·서버 기록은 그대로다. 화면이 "전부 되돌렸다" 로 읽히면 아이는 복구됐다고 믿고 그 위에 쌓는다. `notCoveredNotice()` 가 그 문장을 소유하고 승인 모달과 완료 안내가 같은 문장을 쓴다; ⑩ 문은 명령(`hypeproof-chat.restoreCheckpoint`, "방금 것 되돌리기")과 채팅 패널 툴바 버튼 **하나** — 강사가 자리에 와서 누르는 것도 같은 문이다. **이 행이 주장하지 않는 것**: 원격 강사 복구 아님(#673 3번 — 자리에 와야 한다), 웹뷰 안의 아이용 버튼 아님(툴바 아이콘까지), 외부 배포 롤백 아님 | U (`extensions/hypeproof-chat/test/workspace-checkpoint.smoke.mjs` — 실제 파일시스템 위의 양성 대조군(해시 동일 복원)과 음성 대조군 7종: 삭제 미계획 · 미변경 과대계상 · 건너뛴 파일 은폐 · 전부 복구 주장 · 심볼릭 링크 추종 · 보관본 삭제 · 없는 저장점의 조용한 성공) |
...
| REQ-Q9 | 업로드는 명시 트리거만 (#596·#607) | **채팅 패널 상시 버튼**("📮 기록 보내기", #607 — opt-in(`analytics.upload_session_logs === true`) 코호트에서만 렌더, OFF 에서는 렌더 0) · 커맨드 팔레트("HypeProof: 오늘 활동 기록 보내기") · **세션 종료 감지 배너**(proxy 런타임의 `session_window`, 활성화당 1회) · **기동 시 잔여분 배너**(미업로드 세션 발견 시 — agent-sdk 런타임은 session_window 를 못 보므로 이 경로가 주력) — 어느 쪽이든 버튼 클릭 없이는 어떤 업로드도 발생하지 않는다. 트리거 시 현재 작업 폴더의 실제 `index.html`을 `session_end` snapshot으로 기록한 뒤 세션을 **봉인(seal)** 하고 올린다 — 마지막 응답 뒤 직접 편집도 보존한다. 봉인 없이 "활성 세션 제외"만 있으면 1일차 수업 끝에 오늘 데이터가 하나도 안 올라간다(1회차 리뷰 F1). 재진입 락으로 이중 실행 방지 — 패널 버튼도 **같은 커맨드**를 부르므로 락·토큰·opt-in 게이트를 팔레트와 공유한다(버튼을 안 보는 좌석이 웹뷰 메시지를 직접 보내도 커맨드가 fail closed 로 거절). 배너만 있던 동안 초등학생 코호트는 배너를 지나치면 재진입점이 팔레트뿐이라 사실상 도달 불가였다(#607). 프로필 확인 실패(네트워크)와 opt-in 미설정은 **다른 문구**로 구분한다(꺼져 있다는 오보 금지) | E (`e2e/tests/28-upload-entrypoint` — 실기기 ON/OFF 대조) · U (`test/session-upload-entrypoint` — 웹뷰↔호스트 게이트 드리프트 락) |
