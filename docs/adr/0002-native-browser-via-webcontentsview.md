...
- `WebContentsView`는 메인 프로세스 자원이므로 확장에서 접근할 수 없다 → **vscodium-base
  코어 패치**(`patches/`)로 메인 프로세스 surface + IPC를 노출한다.
...
- 메인 프로세스 코어 패치가 필요하다(단순 확장 작업이 아님). `WebContentsView`는 DOM이
  아닌 네이티브 레이어라 에디터 영역 위 **positioning/z-order/포커스**를 직접 관리해야
  하고, 검증은 1–2h full build를 수반한다.
