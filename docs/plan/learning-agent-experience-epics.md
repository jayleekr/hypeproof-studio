...
| 3 · E7 #755 + E3 #749, D1/D2 | 현재 실제 모델 표시 → 검증된 조합의 제한 선택 → 세션 재개·취소·예산 확인 순으로 묶는다. | 판단에 필요한 정보. 이름, 모델, 실행기, 권한은 별개. 요청한 alias와 실제 provider/model의 차이·미지원 이유를 확인. AE-12~17/27~29 |
