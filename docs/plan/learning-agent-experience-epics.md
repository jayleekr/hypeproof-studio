...
| U1b 종류별 회수 + 재시작 전 세션 — **구현 · 로컬 합성·workerd D1·브라우저·실제 Mac 1대 실행 · 인수 전 · 운영 비활성**(2026-09-22, migration 0025 가산, 인수 ID AT-48 — [계약](../requirements/classroom-admin.md#remote-management-u1b-20260922) · [실행 기록](../testing/classroom-admin.md#remote-management-u1b-run-20260922)) | 선택 회수에 종류(수업 기록 / 학생 프롬프트 / 학생이 승인한 결과물)를 붙이고, 같은 학생·같은 회차 창의 세션을 세션별 파트(schema /3)로 모아 Service가 종류·범위를 다시 판정한다. 원문 열람·기준별 보고서 분할·마무리 경로의 여러 세션은 미구현 | 중 | 승인 명령의 학생 화면 위치, 아동 보호자 동의(#1175) |
...
#### U3 설계 계약과 구현 순서 · 2026-09-21 (구현됨 · 인수 전 — 원격관리 전체나 운영의 완료가 아니다)
