...
|---|---|---|
| App | 기존 채팅 표시, local filesystem/browser/SDK 실행, 승인·중지, 로컬 복구 | 기존 ChatPanel·sdkCoach·browserMcp. 수업 내용/임계값을 바이너리에 고정하지 않음 |
| Service | 인증·배정·정책 해석·예산·실행 자격·버전 binding | 기존 profile/authoring/classroom API 확장. 새로운 로그인·정책 저장소를 먼저 만들지 않음 |
| Surface | 기능 템플릿 편집·학생 조건 리허설·운영 현황·예외 처리 | Chalk authoring/board/classroom 및 #732 재사용 |
| Module | 목적·과제·교수 전략·검수 기준·필요 기능의 참조 | 기존 hps-module/1·session-design 진화. 콘텐츠 자체는 권한을 부여하지 않음 |
