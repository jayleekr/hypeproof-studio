...
- 드롭다운과 직접 입력은 **같은 설정 한 칸**(`연결별 선택 모델`)을 바꾼다. `/model` 슬래시 명령은 만들지 않는다
...
- 비용: 기존 사용량 기록(`captureUsageCost`·`persistUsage`)을 강사 몫으로 남긴다. 학생 예산·이용권에서 빼지 않는다
...
- 색·글꼴·간격은 **기존 CSS 변수(`--vscode-*` 와 `styles.css` 의 토큰)만** 쓴다. 새 색 값(hex·rgb)을 코드에 넣지 않는다
