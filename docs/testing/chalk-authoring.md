...
## 리허설 진입/복귀 — Studio 면 쪽 절반 (#1205, RUN-01 · ARC-03)

`npm --prefix e2e run test:rehearsal-surface` 는 **진짜 Studio 창에서 실제로 눌러서**
교환권 발급 → 교환 → 학생 조건 전환 → 복귀까지 돈다. 스텁 서버가 곧 관측점이라
**앱이 내보낸 `authorization` 을 전선에서 읽는다.** 12건 통과.

세 구간의 자격이 전선에서 확인됐다 — 발급은 issuer Bearer, **교환은 인증 헤더 없음**
(교환권은 본문에만), 그 뒤 프로필·채팅은 좌석 토큰. 리허설 중 issuer 로 나간 요청 0건.
`extensions/hypeproof-chat/test/rehearsal-session.smoke.mjs` 가 같은 판정을 순수 함수와
**구조**로 다시 잰다: `ISSUER_TOKEN_KEY` 를 읽는 파일이 정확히 5개이고 `proxyClient` ·
`chatPanelProvider` · `sdkCoach` 는 거기 없다. "issuer 를 안 싣는다" 가 규율이 아니라
구조라는 뜻이다.

**덜 주장하는 것 하나**: 전환 직전에 출발한 요청은 전환 뒤에 **직전 자격으로 착지할 수
있다**(`loadAccess()` 가 요청 시작 시점에 토큰을 읽는 기존 경합). issuer 유출이 아니고 이
슬라이스가 만든 것도 아니다. 그래서 "리허설 중 모든 요청이 좌석 토큰" 이라고 쓰지 않고
**범위를 사용량 조회로 못 박는** 단언을 뒀다 — 대화 경로는 깨끗하다.

**이 슬라이스가 증명하지 않는 것**: 서버 쪽 교환·게이트(#1189)와 로스터 우회 방지는 81 의
시험 15종이 잰다. 여기 스텁은 그쪽 **코드를 읽어서** 맞춘 것이지 실물 worker 가 아니다 —
이 브랜치 diff 에 worker 변경이 0건이라 `wrangler dev` 짝을 주장하지 않는다. `#1189` 머지
뒤 둘을 붙여 한 번 더 도는 것이 남았다. 리허설 증거 적재(#1202)와 발급 차단(#1190)도 범위 밖이다.

**계측기 오판 둘을 기록한다** — 둘 다 이 작업에서 잡혔고 둘 다 통과 쪽으로 틀리는 종류다:
① 팔레트 목록을 `querySelectorAll` 로 긁으면 **가상화 때문에 렌더된 행만** 나온다(학생 창
23개 중 17개). 선언된 총계(`aria-setsize`)를 함께 읽고 **못 미치면 던지도록** 바꿨다.
② 그 전수 훑기가 포커스를 목록 끝에 두는 바람에, 상대 이동으로 명령을 고르던 기존 코드가
엉뚱한 항목을 실행했다 — **채팅 컨테이너 마운트 대조 검사가 즉시 잡았다.**

### 짝 실행 — 진짜 worker 를 물렸다 (#1205 × #1189)

`HPS_PAIR_UPSTREAM` 을 주면 로컬 서버가 **스텁이 아니라 기록하는 중계기**가 된다.
행동은 진짜 worker 가 정하고 관측만 이쪽이 한다. 좌석 토큰도 합성하지 않고 **교환
응답에서 배운다.**

2026-09-21 실행: worker `feat/1189-rehearsal-entry-survey` @ `f7cce6f`, 포트 8796
(`wrangler dev --local` — Cloudflare 에 아무것도 가지 않는다), fixture
`boah-dental-2026-a / pair-course / m2026.09.21-9`, 서명된 실제 issuer·학생 토큰.
**12/12 통과, 연속 2회.**

**짝을 물리자 양쪽에서 제품 결함이 하나씩 나왔다. 둘 다 그 전까지 초록이었다.**

| 어디 | 무엇 | 왜 안 보였나 |
|---|---|---|
| 앱(이 슬라이스) | 교환권 발급 경로가 `/v1/authoring/…` 였다. 실물은 `/admin/cohorts/…/authoring/…` | 스텁이 `url.includes("/rehearsal-tickets")` 로 느슨하게 받아 줬다 |
| worker(#1189) | 리허설 좌석의 `/v1/profile` 이 `403 not_in_roster` — 진입 첫 단계가 막혔다 | 채팅 한 표면만 재고 다른 표면을 짐작했다 |

**스텁은 내가 기대하는 서버를 흉내내므로 내 오해까지 같이 흉내낸다.** 그래서 스텁
모드의 초록은 **계약 준수의 증거가 아니다.** 스텁 모드는 worker 없이 도는 값 때문에
남겨 두되 그 한계를 파일 머리에 적었다.

세 번째도 짝에서 나왔다: `expires_at` 이 **발급 응답에서는 교환권 수명(24h), 교환
응답에서는 좌석 수명**이라 같은 이름이 위치에 따라 뜻이 바뀐다. 만료를 화면에 쓰려면
**`credential_expires_at` 만** 읽어야 한다(81 이 두 응답 모두에 넣었다).

**단정 하나를 되돌린 것도 기록한다.** "복귀 뒤 좌석 토큰으로 더 나가지 않는다" 는
한 번 통과하고 한 번 `3 !== 2` 로 깨졌다 — 전환 경합의 **거울상**이라 타이밍에 따라
갈린다. 없는 것처럼 쓰지 않고 **"이전 자격으로 정착한다"**(마지막 프로필 조회가 이전
자격)로 바꿨다. 강사가 실제로 겪는 것이고 경합과 무관하게 참이다. 늦게 착지한 좌석
조회 수는 `report.json` 에 세어 남긴다(두 실행 다 0).
