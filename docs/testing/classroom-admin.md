...
‘현황 새로 확인’)으로 줄이고, 신호 칸을 넓혔다(`3f2538d`). 그래도 CI에서 7행이어서 선택 문장을 조치 줄 앞으로 옮기고 오래된 신호는 시각 대신 ‘오래됨’만 쓰게 했다(`4082190`, CI 8행). 합성 미리보기(배포 켜짐)에서는 ‘보내기’·‘작성’이 셋째 줄로 밀려 7행이 되어, 작성 항목을 고르기 줄로 옮기고 G1 시험 픽스처도 배포를 켠 상태로 바꿨다(G1 시험이 배포 꺼진 수업만 재서 놓쳤던 경우). 학교 Windows PC의 글꼴에서도 같은 종류의 차이가
