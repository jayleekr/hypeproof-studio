...
| 실제 Mac | 정상 선택 회수는 직전 빌드(`586d13e`)에서 확인했고 이번 변경은 **종료된 요청 뒤의 바이트 해석**과 멱등 재전송의 시각 기록뿐이다. 정상 경로(실행 중 → 검증됨)는 이 행들을 지나지 않으므로 같은 실기를 반복하지 않았다. 열려 있던 시연 세션은 이전 빌드라 그대로 두지 않고 새 빌드로 다시 띄웠다(아래) | 세션 재시작만 |
