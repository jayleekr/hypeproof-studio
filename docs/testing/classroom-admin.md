...
- 확장 `test/evidence-snapshot.smoke.mjs`(4 PASS, 실제 Service 경로 in-process): allowlist 파일만 전송, verified receipt 뒤 재전송 0, 전송 중 오프라인→pending→**같은 고정 사본**으로 재개(그 사이 커진 spool 미반영), 변경된 bytes는 r2·input revision 2, 혼입/무기록/철회 사유 보고.
