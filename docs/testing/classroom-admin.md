...
| AT-46 | ADM-10/11 · RM-3/5 · AE-09/12/26/33 · VER-01 · INT-CO-04(제안) | 수업 **설정**(확정 강의 버전)의 대상 배포: 선택/비선택, 실행 중 변경, 두 runtime, 전환 커밋 경계의 경합, 재발급·좌석 교체·다른 창·늦은 응답, OFF·종료·권한 폐기·정책 축소, 기준 변경, 회수와 복귀, 미지원 App | 진행 중 turn은 시작 때의 설정으로 끝나고 **다음 실제 모델 요청**에서 App(`/v1/profile`)·Service(gate)·요청 헤더가 같은 binding key, `적용됨`은 Service가 본 그 첫 요청뿐(준비·전환·HTTP 200은 아님), 비선택 학생의 실행·profile 불변, 허용 범위 밖·권한 확대 0, 이전 기준의 자기보고·강사 확인을 새 기준 완료로 상속 0, 회수가 설정을 되돌리지 않고 복귀는 새 요청+새 적용 증거, 예전 버전 자동 부활 0 | Service + App + 합성 upstream + UI e2e + 실제 Mac ([시나리오 S1~S17·M2](#remote-management-u3-plan-20260921)) |
...
| AT-45 | 수업 프롬프트 대상 배포 | **NOT RUN · 계획** — 설계 계약만(코드·시험 파일 없음) | [U3 인수 계획](#remote-management-u3-plan-20260921) |
| AT-46 | 수업 설정 대상 배포 | **NOT RUN · 계획** — 설계 계약만. 적용 시점의 운영 정책은 사용자 결정 대기(로컬 가정: 다음 질문부터) | 같은 곳 |
...
### U3 — 수업 프롬프트·수업 설정 대상 배포: 인수 계획 · 2026-09-21 (전부 NOT RUN · 계획)
...
| **S1** | v2 설정을 A1·A3에 배포, A2 비선택. A1은 다음 질문을 보내고 A3은 보내지 않음 | A1: `준비됨` → (질문) → `전환 기록됨` → `적용됨`. **합성 upstream이 받은 A1의 그 요청**: `system`에 v2의 단계(`s4` 있음·`s2` 없음)와 v2 이름, `model` = v2 기본값, proxy면 `tools`에 빠진 기능 없음. `/v1/profile`: `lesson.version=v2`·`sdk_tools.write=false`·`lesson_binding.key` = 그 요청의 응답 헤더 `x-hps-lesson-binding` = binding 행의 key(**세 곳이 같은 key**). A2: 같은 시각의 요청이 v1 · binding 행 0 · `/v1/profile` 응답이 배포 전과 바이트까지 같음. A3: `준비됨`에 머물고 **`모두 적용`이 나오지 않음** | Service + App host + 합성 upstream + 브라우저 e2e | NOT RUN · 계획 |
| **S2** | **실행 중 설정 변경**: A1의 SDK turn이 여러 `/v1/messages` 요청을 보내는 도중에 v2를 확정·준비시킴 | 그 turn의 **남은 모든 요청**이 시작 때의 key(v1)로 나가고 upstream이 받은 `system`·`model`이 끝까지 v1, 응답·대화·도구 호출 중단 0. turn이 끝난 뒤의 다음 질문부터 v2. **음성 대조군:** turn 고정 헤더를 끄는 시험 전용 스위치에서는 같은 단언이 **실패해야** 한다(계측기가 중간 변경을 잡는지 확인) | 실제 SDK binary + 로컬 Service + 합성 upstream | NOT RUN · 계획 |
| **S3** | 두 runtime 각각(proxy 좌석 · agent-sdk 좌석)에서 S1. 헤더는 추측하지 않고 upstream 앞의 기록 서버에서 **실제로 나간 값**을 읽는다 | 두 runtime 모두 turn의 모든 요청에 같은 key 헤더, Service의 응답 헤더와 일치. runtime이 다른 버전 vR은 저장 단계에서 거부(S14) | 실제 SDK binary/proxy client + 로컬 Service | NOT RUN · 계획 |
| **S4** | App–Service 일치: 전환은 성공했는데 profile 후보 확인이 실패(5xx·timeout 주입) / 그 상태로 유예 10분 경과 | 그 turn은 v1 key로 나가고 Service가 직전 binding으로 실행 — **도구 정책(App)과 system prompt(Service)가 같은 v1.** 유예 뒤: `409 lesson_binding_changed`, 모델 요청 0, 입력 글·첨부가 입력창에 복원(`inputRejected`), 안내 문구. 전환·profile이 모두 실패한 경우 학생의 turn은 v1로 정상 진행되고 강사 화면은 `준비됨` 그대로(‘적용’ 0) | App host(fault 주입) + Service | NOT RUN · 계획 |
...
| **S6** | **전환의 커밋 경계**(읽기 뒤·조건부 INSERT 전에 끼어듦): 배포 회수 · `ops_lesson_settings` OFF · 회차 종료 · 좌석 교체 · 재로그인(epoch+1) · lease 이전 · 더 새 v3 확정 · 배포 강사 fence | 8종 모두 binding 행 0 · 감사 0 · `recorded:false`와 맞는 사유 · 그 학생의 다음 요청은 직전 버전. 조건이 돌아오면 다음 turn 경계에 같은 의도가 전환됨. **응답은 batch 성공이 아니라 INSERT의 `meta.changes`에서 만든다** — batch는 성공했는데 0행인 경우에 `recorded:true`가 나오면 실패 | Service(경합 hook) | NOT RUN · 계획 |
...
| **S9** | 다른 학생·수업·회차: A1의 key를 A2의 토큰 요청에 헤더로 · A1의 새 회차 · 다른 cohort 강사 · 학생/ops 자격으로 강사 API | A2 요청 `409`(A2의 현재·직전 key가 아님 — **헤더가 강의를 고르지 못함**), 새 회차는 토큰 강의로 시작(binding 0), 범위 밖 강사·학생 자격 거부, 모든 경우 새 테이블 행 수 불변 | Service negative | NOT RUN · 계획 |
| **S10** | 재발급: 같은 버전으로 재발급 · 다른 버전(v3 토큰)으로 재발급 · 전환 요청의 `base_lesson_sha256` 위조 | 같은 버전: binding 유지·다음 요청 v2. 다른 버전: 토큰이 이김(v3)·보드 문구·`적용됨`이 남아 있지 않음. 위조: 얻는 것은 자기 토큰의 강의뿐(더 넓은 실행 0) | Service + App host | NOT RUN · 계획 |
| **S11** | **기준이 바뀔 때**: v1에서 `s1`·`s2` 자기보고(`submitted`) + 강사 확인(`confirmed`) → v2 전환. 전환 뒤 늦게 도착한 v1 단계 사건. v2 좌석에 `mark_checkpoint(s2)`. 회수 snapshot을 봉인 | 보드: v1의 단계·확인은 `이전 기준(v1)의 기록`으로만 보이고 **v2의 `s1`(같은 id)이 제출·확인으로 표시되지 않음**, v2는 ‘아직 시작 전’. 늦은 v1 사건 `lesson_mismatch` 격리(보드 불변). `mark_checkpoint(s2)`는 v2 기준 `unknown_step`. App 완료 게이트가 v1 사건으로 v2 단계를 닫지 않음. spool에 `lesson_binding` 사건이 새 기준의 첫 `prompt` **앞**에 있음. seal: `foreign_activity` **아님**, 그 학생 입력에 `mixed_lesson_basis`. **양성 대조군:** binding이 없는 A2의 입력에는 표시 없음 | Service + App host + 브라우저 e2e | NOT RUN · 계획 |
| **S12** | **회수 ≠ 복귀**: ① 전환 전에 회수 ② 전환·적용 뒤에 회수 ③ 복귀(`base:true`) 배포 ④ 예전 v1을 가리키는 새 revision 배포 | ① 대상 `revoked`·다음 요청 v1·binding 0 ② **회수 직후의 다음 요청이 여전히 v2**(upstream에서 관측), binding 행 불변, 안내 카드만 내려가고 화면은 ‘이미 적용된 설정은 그대로’ — **v1이 저절로 되살아난 경우 0** ③④ 새 배포 → `준비됨` → 다음 질문에서 전환 → 그 요청의 `system`이 v1 → 그때 `적용됨`. 복귀 전환 전에는 어떤 화면도 ‘복귀됨’이라 하지 않음 | Service + App host + 합성 upstream + 브라우저 e2e | NOT RUN · 계획 |
| **S13** | 끄기·종료·권한·정책: `ops_lesson_settings` OFF · 전역 `HPS_CLASSROOM_OPS` OFF · 회차 종료 · 배포 강사 폐기(배포 강사 ≠ 연결 발급 강사) · **정책 축소**(전환 뒤 시험용 profile에서 기능·모델 제거) | flag OFF: 새 저장·배포·전환 `disabled`, **이미 전환된 학생은 v2 그대로**, 회수 전달·receipt 정산은 계속. 전역 OFF: 전원 토큰 강의 · `409` 0 · 기존 수업 회귀 0(문서가 말한 대로 turn 고정은 보장하지 않음을 그대로 관측). 종료: 새 요청 `run_ended`. 폐기: 미전환 `revoked`, 전환분 유지. 정책 축소: `409 lesson_unavailable`, **v1이나 더 넓은 집합으로 실행된 요청 0**, 보드에 원인과 조치 | Service + App host | NOT RUN · 계획 |
...
| **S15** | 호환·부분 적용: `lesson_binding` 미선언 App(공지·프롬프트는 받음) · 구 Service + 새 App · **migration 0024가 없는 DB + 새 Service** · 헤더 없는 요청 · 앱 재시작(준비 뒤·전환 전) | 미선언 좌석은 설정만 `unsupported`(부분 적용을 성공으로 세지 않음). 0024 없는 DB: 채팅·`/v1/profile` 정상(binding 읽기 실패가 격리됨), 설정 저장은 `503`. 헤더 없는 요청은 현재 binding으로 실행. 재시작 뒤 `pending_binding`이 남아 재연결 뒤 다음 경계에 전환. **같은 grant의 자동 재연결은 U2와 같이 NOT RUN** | Service + App 계약 + App host | NOT RUN · 계획 |
| **S16** | 비용: binding 없는 회차의 gate · binding 있는 gate · 유예 경로 · `step` 있는 sync · idle sync · 전환 · 보드 · 결과 조회를 30/100/200석에서 `meta.rows_read/rows_written`·호출당 문장 수로 계측 · 전 좌석 동시 전환 | [요구 문서의 예산](../requirements/classroom-admin.md#remote-management-u3-20260921) ①~⑥ 안. idle sync 증가분 0을 **문장 수로** 확인. 기존 채팅 p95 증가 ≤ 5%. 문장 수를 운영 quota라고 쓰지 않는다 | 로컬 workerd D1 계측. **운영 D1·계정 plan NOT RUN/미확인** | NOT RUN · 계획 |
| **S17** | 회귀: flag 전부 OFF · `ops_distribute`만 ON(U2) · U1 선택 회수 · migration 0024 전/후 fresh vs 누적 · 기존 lesson 좌석(binding 0)의 gate·`/v1/profile` 응답 | 기존 suite·브라우저 e2e·U1/U2 독립 재현 그대로 PASS. **binding이 없는 좌석의 `/v1/profile` 응답은 `lesson_binding` 한 키 외에 바이트 동일**, system prompt 동일. schema 동등·재적용 멱등 | regression + 로컬 workerd D1 | NOT RUN · 계획 |
| **M2 실제 Mac** | 실제 Studio 창(A1 · 공식 shell 복사본 + 현재 확장 + 실제 SDK binary) + 실제 Chalk `/manage`(보이는 브라우저) + 합성 A2·A3 + 합성 upstream. **프롬프트:** 작성 → A1 선택 → 보내기 → 학생 창에 글과 첨부가 있는 상태에서 `초안에 가져오기` → 되돌리기 → 다시 가져와 편집(보내지 않음) → 회수. **설정:** 확정 v2 고르기 → 미리 확인의 영향 요약 → 보내기 → 학생 안내 카드 → **학생 turn이 진행 중일 때 확정되도록** 맞춤 → 그 turn 종료 → 다음 질문 → 강사 `적용됨` → 회수 → 다음 질문(여전히 v2) → `복귀 보내기` → 다음 질문 → v1 `적용됨` | 강사 화면의 대상별 단계 시각(준비·전환·적용), 학생 화면의 단계 목록·AI 이름이 v2로 바뀐 시점이 **다음 질문**임, 진행 중이던 turn의 upstream 기록이 끝까지 v1, 가져오기 전후 초안·첨부·대화·workspace hash, 회수 뒤 초안 그대로, 합성 A2 binding 0·profile 불변 · A3 `준비됨`. 증거: source SHA · shell/SDK 버전 · `result.json` · 화면(강사/학생 각 단계) · upstream 기록의 요청별 key·`system` hash · binding 행 덤프 | 실제 Mac GUI | NOT RUN · 계획 |
