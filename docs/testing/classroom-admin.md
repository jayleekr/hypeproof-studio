...
U3 잔여 오류 → U4 원인별 조치 → 필요한 회수 흐름(U1b)은 2026-09-22 로컬 인수까지 왔다(외부 환경 NOT RUN).
...
### G1 강사 운영 화면 검사 · 2026-09-22

[디자인 G1](../requirements/classroom-design.md#g1-instructor-ia-20260922)의 인수 기준을 실제 브라우저에서 확인한다. CSS·문자열 존재가 아니라
**행 상자 측정과 조작 결과**를 본다. 캡처는 사람이 보는 인수 그림이며 아래 조작 검사를 대신하지 않는다.

| 검사 | 조건 / 깨뜨릴 가정 | 합격 기준 | 실행 |
|---|---|---|---|
| G1-T1 표 밀도 | 합성 24석, 펼침 없음, 확대 100% | 1280×720 완전한 행 ≥8, 1024×640 ≥4 (행 상자가 화면·목록 상자 안에 모두 들어온 것만 셈), 표 글자 ≥14px, 문서 가로 넘침 없음, 7열 동일 | `e2e/classroom/ops-g1-layout.mjs` ① + 캡처 육안 |
| G1-T2 상태 구분 | 토큰 거부·도움 요청·오래된 신호·승인 대기·공통 장애 | 빨강은 확인된 차단만, 도움 요청은 ✋ 별도 줄, 무신호·오래됨은 ?·중립, 승인 대기는 오류 아님, 긴 학생 ID는 칸 안 | ① |
| G1-T3 필터·찾기 | 선택 후 필터로 선택 학생을 가림 | 보기만 바뀜, 가려진 선택도 대상·수 표시, 명령 0건 | ② |
| G1-T4 주요 버튼 | 선택 없음 / 문제 좌석 / 정상 좌석 | 각각 문제 좌석 고르기 / 진단 / 회수가 유일한 주요 버튼, 누르면 확인 화면만(실제 회수·명령 0), 기본 선택 종류 없음, 메뉴는 고른 뒤·Esc로 닫힘 | ③ |
| G1-T5 상세 | 1280 옆 상세, 1024 서랍 | 1280: sticky로 목록 옆, 선택·현재 행 유지, 원인·다음 조치·기술 ID는 상세에. 1024: fixed 서랍, Esc로 닫고 연 행으로 포커스. 화살표·Home·End·Enter, 행 클릭 | ①④ |
...
**CI(ubuntu, 다른 한글 글꼴)에서 드러난 것:** 첫 PR head `6828f99`에서 같은 G1-T1이 1280×720 **6행**으로 실패했다(첫 행 y=407, Mac y=327).
글자 폭이 넓은 글꼴에서 ‘현황 새로 확인’이 둘째 줄로, 선택 문장이 따로 한 줄로 밀리고 ‘25분 전 · 오래됨’이 세 줄이 됐다. 버튼 표시 문구를 ‘새로 확인’(접근 이름은 그대로
‘현황 새로 확인’)으로 줄이고, 신호 칸을 넓혔다(`3f2538d`). 그래도 CI에서 7행이어서 선택 문장을 조치 줄 앞으로 옮기고 오래된 신호는 시각 대신 ‘오래됨’만 쓰게 했다(`4082190`, CI 8행). 합성 미리보기(배포 켜짐)에서는 ‘보내기’·‘작성’이 셋째 줄로 밀려 7행이 되어, 작성 항목을 고르기 줄로 옮기고 G1 시험 픽스처도 배포를 켠 상태로 바꿨다(G1 시험이 배포 꺼진 수업만 재서 놓쳤던 경우). 배포를 켠 CI에서 다시 6행이 되어 ‘다른 조건으로 선택’을 ‘다른 조건’(접근 이름 유지)으로 줄이고 선택 문장이 조치 줄의 남은 폭을 쓰게 했다. 로컬에서 `HPS_G1_WIDEN=0.07`(모든 글자 간격 +0.07em)로 넓은 글꼴을 흉내 내도 8행이 유지됐다 — 실제 Windows 글꼴 측정은 아니다. 학교 Windows PC의 글꼴에서도 같은 종류의 차이가
생길 수 있으며 **Windows 실측은 하지 않았다**.
...
<a id="g1-wrap-state-run-20260922"></a>

#### G1 마무리 단계 판정 보정 실행 기록 · 2026-09-22 (Mac arm64 · Chromium · source `87654c5` + 배지 클래스 이름 보정)

독립 검토가 G1 인수를 **마무리 단계 표시 결함 하나로 보류**했다(레이아웃 전체 반려 아님 — 1280×720 8행·1024×640 6행·선택·서랍·결과 카드 검사는 검토자 재실행에서 통과).
판정 규칙은 [디자인 G1 마무리 단계 판정](../requirements/classroom-design.md#g1-wrap-state-20260922).

| 실행 | 결과 | 범위 |
|---|---|---|
| `test:classroom-ops-g1-wrap`(신규, A–G 8건) | 통과 | A만 직접 렌더(합성 표시 픽스처). B–G는 Chalk 페이지 + Service router + SQLite의 실제 회수 올리기·봉인, 초안 작업·검수, 발송 승인·요청, 운영자 전달 이벤트 route. 합성: 계정, 기기 올리기(하네스가 App 동결기로 만든 파일), 모델·메일 공급자(전송 지점 대체), ‘전달됨’ 이벤트. S4의 연결 만료는 SQLite에서 만료 시각을 과거로 옮긴 것 |
| 음성 대조 — 같은 스위트를 수정 전 `122563a`의 `manage.html`로 | 첫 검사에서 실패(상태 단어·`data-state` 자체가 없음) | 거친 대조다. 결함의 실질 반례는 검토자 프로브(`pending-wrap-probe.json`)다 |
| `test:classroom`·`-ops`·`-ops-roster`·`-ops-selection`·`-ops-distribution`·`-ops-lesson-settings`·`-ops-help`·`-ops-results`·`-ops-g1`·`-report` | 통과(roster는 첫 실행 1회 실패 뒤 재실행 통과 — 아래) | 기존 시험에서 바꾼 것은 G1 마무리 칸의 클래스 이름(`wrap-idle`/`wrap-off`)과 상태 단어 확인뿐 |
| `npm --prefix chalk test`, `npm --prefix chalk run typecheck` | 통과 | Chalk 계약 |

`ops-roster` 첫 실행이 ‘근거·조치’ 키보드 열기(마무리 이전 단계, 이번 변경이 실행되지 않는 경로)에서 한 번 시간 초과됐다. 반복 측정은 인계 기록에 남긴다.
**NOT RUN:** 실제 Mac 여정 재실행(바뀐 것은 `/manage` 마무리 칸 표시와 응답 묶음 판정뿐이라 Studio 앱 경로와 무관), Windows, 실제 모델·메일 수신, 사용자 시각 인수.
...
| AT-41 | 세 흐름 공통 결과 화면 | **구현 · 로컬 실행 PASS · 인수 전(2026-09-22, 보정 U4c `441e185`)** — Service + 브라우저 e2e(18: 12건 초과 보존·다시 열기·숨은 탭·수동 읽기 경합·수업 마무리 포함) + 실제 Mac 창 1대(진단→회수→공지→수업 마무리). 수업 설정(U3) 카드 실제 창·Windows·학교망·복수 기기·운영 D1 NOT RUN | [AT-41 실행 기록](#remote-management-at41-run-20260922) |
...
| R3 | 보존형 초기화 + 그 뒤 공급자가 500 | 초기화 = `실행만 됨 · 비용 없는 준비 확인`, 초기화 자체의 모델 호출 0, 초안·파일 동일. 다음 질문 실패 → **문제 남음 · 원인 AI 제공자 장애(`provider_5xx`) · 다음: 공통 장애입니다 — 이 PC를 다시 초기화하지 마세요**(조건 없이 단언). 학생 앱의 turn 기록(spool `turn_end`)도 같은 실패(`status: error`). 그 뒤의 정상 실행은 `ok`로 기록되고 그 조치의 판정을 고쳐 쓰지 않음(1회 응답) | 스트림·순수 사슬 · 기기 host · **실제 Mac** | PASS (8~12차) — 실제 SDK는 11회 재시도 뒤 **예외를 던져** 끝냈다(아래 ②) |
| R4 | 미리보기: 무관한 localhost 도구 탭(**먼저** 연 탭)·외부 사이트 탭·학생의 **실제 미리보기 탭**(실제 live server). ① 학생 페이지를 치움(404) ② 되돌림 ③ 앱의 미리보기 서버가 죽음(시험 전용 `HPS_TEST_PREVIEW_FAULT`) | ① `문제 남음 · 결과물 페이지가 없음`, 탭 이동 없음 ② 같은 주소에서 학생 탭이 페이지를 새로 불러옴 → 해결 확인 ③ **같은 탭 target**이 새 포트의 같은 경로로 옮겨지고 그 탭의 문서에 학생 페이지 고유 내용이 있음(`readyState` complete · visible) → `새 주소로 다시 연결했고 결과물 페이지가 열림`. 세 경우 모두 무관한 두 탭의 target id·주소 불변, 무관 도구 서버가 받은 요청 0, 파일 동일 | 순수 절차(양/음성) · 실제 `BrowserControl`(가짜 vscode/CDP) · 기기 host · **실제 Mac** | PASS (8~12차, 두 결함 수정 뒤 13차 R0+R4). 음성 대조: 같은 복사본을 변수 없이 띄우면 같은 트리거 파일이 아무것도 하지 않음(`mac-preview-fault-negative.mjs`) |
...
| R7 | 토큰: 기존 발급분 재확인 → 강사가 **`/authoring` 참여 코드 발급 화면**(보드의 좌석 링크가 강의 수업을 보내는 곳)에서 클릭으로 재발급 → 입력 전 재확인 → 학생이 **앱의 시작 화면**(‘활동 변경’ → ‘다른 활동 선택’ → ‘수업에 참여하기’ → 코드 붙여넣기 → ‘코드 확인하기’ → 시작)에 입력 → 재확인 | 세 답이 다름: `기존 발급분 재확인` / 입력 전 `문제 남음 · 새 토큰이 학생 앱에 없음` / 입력 후 `재발급한 새 토큰 활성화`(앱이 확인한 발급 ID = 발급 화면에서 만든 것). dev token file 불변·앱 재시작 없음, **같은 작업 폴더**, 입력 중이던 초안·대화·파일 유지, 비선택 학생 발급 0, 미연결 좌석은 ‘연결 코드 발급’이 첫 단계(원격 전달 없음). 토큰 원문은 보드·발급 화면(지운 뒤)·원장·증거에 없음 | Service · 기기 host · 브라우저 · **실제 Mac** | PASS (12차). 새 발급은 이전 연결 세대를 끊으므로(기존 fence 계약) 입력 뒤 학생 PC가 강사의 연결 코드로 다시 연결했다 — 조치 전달 경로의 사실로 기록 |
...
| 실제 host(VS Code만 stub) ↔ 가짜 Service · 미리보기 대상 절차 · 재진입 폴더 · 고정 탭 불러오기 | `extensions/hypeproof-chat/test/classroom-ops-recovery.test.mjs`(15) — `npm test`에 포함. 고정 탭: 실제 `BrowserControl` bundle에 가짜 vscode/CDP — 안정된 학생 탭(양성)은 모든 CDP 호출이 그 탭에만 감, 첫 문서 읽기 중·이동 중·새 문서 대기 중 탭 소멸, 세션 닫힘, 같은 주소의 다른 탭 객체로 교체(음성 5종)는 모두 null이고 다른 탭 호출 0, 코치의 활성 탭 대체 동작은 그대로. 여러 학생 페이지: 하나가 404·503·응답 없음이면 탭 이동 0·해결 아님, 모두 정상이면 각자 경로로 loaded, 무관 탭·도구 서버 접촉 0. **두 결함 시험은 수정 전 소스(`5051a01`)에서 실패함을 확인** | PASS |
| SDK 종료 → 매핑 → 판정 사슬 (실제 `runSdkCoach`에 각본 `query()`, 모델·binary·네트워크 없음) | `extensions/hypeproof-chat/test/sdk-terminal-failure.test.mjs`(7): 정상 완료 · `result` 오류(`is_error`/`error_during_execution`) · 재시도 500/429 뒤 멈춤 · **재시도 뒤 SDK가 예외** · 근거 없음 = `unknown` · 앞 turn status 비재사용 · CLI 합성 API-error 줄 | PASS |
...
| **실제 Mac M3** | `e2e/classroom/mac-recovery.mjs` | **PASS — 12차 실행, 확장 source·build `163e0d8`(실행기는 그 위의 대기 조건 수정 1건), R0~R7 전부.** 이전 PASS는 6차(`aa1c05b`). 7~11차 실패 사유: 제품 2(⑦ 아래 ② 예외 경로 · ⑧ 재진입 폴더) · 계측기 3(새 페이지 생성 방식 · 접힌 발급 화면 구역 · 닫힌 시작 화면 소켓 대기). 각 실패 폴더를 사유 이름으로 보존 |
| **실제 Mac R4 재확인** | `HPS_U4_SCENARIOS=R4` (R0 + R4만 — R3의 공급자 대기는 영향이 없어 반복하지 않음) | PASS — 13차, 확장 source·build `2a25816`: 같은 탭 target이 새 포트의 `/page.html`로(문서 complete·visible·학생 페이지 고유 내용), 무관 localhost·외부 탭 id·주소 불변, 도구 서버 요청 0 (`recovery/result.json`의 `scenarios_run`) |
| 미리보기 fault 음성 대조 | `e2e/classroom/mac-preview-fault-negative.mjs` (같은 복사본, 변수 없이, debug 9442) | PASS — 같은 트리거 파일을 만들어도 미리보기 서버·탭 주소 불변, 파일 미소비 (`preview-fault-negative.json`) |
| 이번 보완(2026-09-22, 인수 이관 뒤)의 회귀 | 확장 `npm test`(전체) · `tsc` · Chalk `test:classroom-ops` · 브라우저 e2e `ops`(좌석 링크 기대값 갱신) · worker `classroom-ops-recovery` | 전부 exit 0. 나머지 브라우저 e2e·worker 전체는 PR CI |
| 영향 받은 회귀(최초) | worker `npm test` · `test:classroom-ops`(160 PASS) · `test:classroom-ops:d1` · Chalk `npm test` · 확장 `npm test` · 브라우저 e2e `ops`·`roster`·`selection`·`distribution`·`lesson-settings`·`trial-ux` · worker/확장/webview `tsc` | 전부 exit 0 |

**실제 Mac M3 — 어떻게, 무엇이 실제였나.** 강사의 모든 조치는 보이는 Chromium 창의 **Chalk 페이지 클릭**이다(학생 선택 → 조치 → 확인 → 결과를 그 화면에서 읽음). 학생 쪽은 공식 shell `0.1.56` 복사본 + 현재 확장 build + 실제 Agent SDK `0.3.207` binary의 **실제 창**이며 webview 조작은 실제 마우스 입력이다. 실제: shell 복사본 · 확장 · SDK · ops sync loop·명령 실행기 · Chalk · Service router + SQLite · 앱↔Service 실제 HTTP · live preview 서버와 브라우저 탭. **여기서 만든 것:** 계정·수업·토큰(합성) · 모델 공급자(각본; ‘공급자 장애’ = 500 응답) · `/v1/health` 무응답(로컬 HTTP 앞단) · 학생 페이지를 실행기가 치웠다 되돌림 · 좌석 A2(이 프로세스의 실제 기기 client, 선택 안 함)·A3(미연결)·A4(명령 capability 없음) · 미리보기 서버를 죽이는 시험 전용 트리거(`HPS_TEST_PREVIEW_FAULT`, 음성 대조 있음) · 무관한 localhost 도구 서버(실행기) · 재발급 코드를 발급 화면에서 시작 화면으로 옮긴 것(사람이 읽어 주는 것에 해당, 값은 기록하지 않음). 자기 포트(18841/18842/9441)·자기 user-data·자기 HOME — 설치본, U1/U2/U3 실행기, 사용자의 Studio 데이터는 건드리지 않았다.
...
| ② | 그 실패를 앱은 `unknown`으로 분류했다. 인수 이관 뒤 추적(격리 재현 `u4-sdk-result-snapshot`)에서 더 나쁜 경로도 나왔다: SDK가 스트림을 정상으로 닫고 `result`만 오류면 앱은 **성공 turn**으로 기록해 보드가 `해결 확인`을 냈다. 고친 뒤 7차 실기에서 다시 `unknown` — 실제 SDK는 재시도 끝에 **예외를 던졌고**(spool `error_kind: error`, 응답 줄 `API Error: 502 … (status 500)`) 그 예외에 status가 없었다 | **수정**: 스트림 소비자가 turn 종료를 구조화해 반환(`result` 오류 = 실패 turn — spool·관찰 기록·`closeTurn`·ops 관측 모두 같은 사실, 학생 안내 1회·재시도 없음), 멈춤 오류·SDK 자신의 예외에 **그 turn 스트림이 준 status만** 싣는다(뒤에 정상 응답이 오면 지움, CLI 합성 API-error 줄은 응답으로 치지 않음). 8차 이후 `provider_5xx` + 공통 장애 안내를 조건 없이 확인 |
| ⑦ | 7차 이후: 강의 수업의 좌석 링크가 `/issuer`로 갔다. 그 화면은 강의 없는 토큰을 만든다 | **수정**: 강의가 고정된 수업이면 `/authoring` 참여 코드 발급으로(문구에 강의 버전) |
| ⑨ | Codex 검토(`5051a01`): 고정 탭 불러오기가 시작 때만 탭 생존을 보고, 이후에는 `cdp()`의 활성 탭 대체로 넘어갈 수 있었다 — 첫 읽기 도중 학생 탭이 닫히면 무관 탭에 `Page.navigate`가 가고 성공을 보고함(격리 재현) | **수정**: 그 탭 객체에 붙인 세션 하나로 전 과정을 하고, 탭이 목록에서 사라지거나 세션이 닫히면 즉시 null |
| ⑩ | 같은 검토: 첫 탭의 페이지만 HTTP로 판정해, 학생의 두 번째 탭이 404 페이지여도 전체 `loaded`로 셌다(격리 재현) | **수정**: 탭들이 보일 페이지를 각각 판정, 하나라도 404·5xx·응답 없음이면 그 상태가 결과이고 탭 이동 0 |
| ⑧ | 10차: 재발급 코드를 시작 화면에 넣자 앱이 학생의 현재 폴더가 아니라 **프로필 기본 폴더로 창을 옮겼다**(작업·초안·대화가 화면에서 사라짐) | **수정**: 이 창에 이미 열린 활동과 같은 활동의 코드면 그 폴더를 유지(저장된 기록의 폴더, 기록 이전 창이면 열린 폴더). 다른 활동은 그대로 자기 기본값 |
...
**NOT RUN.** Windows · 학교망·실제 장애 · 실제 복수 기기 · staging/production D1·R2·p95·쿼터 · 실제 모델 · 실제 메일 · Keychain 설치본 · 앱 밖의 실제 원인으로 미리보기 서버가 죽는 경우(시험 전용 트리거로만) · **일시정지를 가로지르는 다중 요청 turn의 실제 창**(Service가 다음 요청을 거절한다는 것은 코드와 Service 시험으로만) · 구 App 실물.

**남긴 것(이번 범위 밖).** 학생별 일시정지 · 승인된 turn의 후속 요청 유예 · 게이트웨이 자체 5xx와 공급자 5xx의 구분(지금은 둘 다 `provider_5xx`, 다음 조치 같음) · 미리보기 상태를 보드에 보고하는 신호 · AT-41의 나머지 · U1b.
...
**NOT RUN (첫 실행 당시 — 아래 보정에서 일부 실행).** 수업 마무리(명단 전체 회수) 카드 클릭 경로 · 실제 창에서 여러 세션이 있는 회수 · 수업 설정(U3) 배포 카드의 실제 창 · 12건 초과로 카드를 내리는 경로 · 숨은 탭 · Windows · 학교망 · 복수 실제 기기 · staging/운영 D1·R2 · 실제 모델 · 메일. 사람 검토: 화면 문구·배치는 사용자 시각 검토 전이며 이 기록은 승인이 아니다.

#### 보정 U4c · 2026-09-22 (소스 `441e185` · Codex 재현 F1 + 남긴 인수 공백)

**F1 재현(Codex, `66bce65`).** 도달 불가 좌석 A6에 진단을 13번 보내면 카드가 #13, #11, …, #1로 남았다 — **#12가 사라지고 #1이 남음**. 원인: `ledgerAdd`가 삽입 순서 Map을 뒤집어 ‘새 카드가 아닌 첫 확정 카드’를 찾아, 가장 먼저 보낸 것이 아니라 **바로 앞에 보낸** 확정 카드를 내렸다. 또 내렸다는 문구가 ‘기록 회수·보낸 자료에서 다시 찾기’를 약속했지만 명령·회수 결과에는 그런 목록 화면이 없다(`GET …/commands/:id`·`…/report-batches/:id`만 있음 — 확인함). 보정: 삽입 순서 그대로 탐색, 내린 카드는 연결 동안 ‘화면에서 내린 결과’에서 id로 다시 열기, 문구는 그 한계를 그대로 말함. 새 서버 원장 없음.

**브라우저 `test:classroom-ops-results` — 18/18 PASS** (위 12 + 아래 6). Codex 원본 재현 스크립트도 이 소스에서 #2~#13을 남기고 #1만 내린다. 음성 대조: 탐색만 `.reverse()`로 되돌리면 보존 시험이 ‘newest first; S1–S3 gone; both pending kept’로 실패한다.

| 확인 | 결과 |
|---|---|
| 수업 마무리 클릭: 미리 확인 = 카드 0·기기 요청 0·실제 batch 0(Service의 dry-run 행 1), 명단 6줄 / 실제 = ‘명단 전체 6명’ 카드 1, A4 동의 없음·A5(바뀐 주인 student-g)·A6 사유 표시, 같은 연결의 선택 회수 K4(A2)와 공존 / 늦게 온 A1 기록이 **그 카드에만** 서버 검증, K4 불변, ‘모두’ 없음 | PASS |
| 숨은 탭(헤드리스에서 `document.hidden` + `visibilitychange`로 흉내): 진행 중 카드가 읽기를 멈추고 ‘가려진 동안 자동 확인을 멈췄습니다’ + 마지막 관측 시각, 그동안 기기가 끝내도 화면은 옛 관측(추측 없음)·해당 GET 0회 → 돌아오면 3초 안에 다시 읽어 적용 | PASS |
| 수동 읽기 경합: 기기 수신 전의 자동 읽기 응답을 붙잡아 두고 수동 ‘이 결과 새로 확인’ → 기기 수신, 붙잡힌 옛 응답이 나중에 와도 되돌아가지 않음, 진행 중 조치는 계속 자동 관측되어 클릭 없이 적용 | PASS |
| 12건 초과 혼합: P1(대기)·S1–S6(확정)·P2(대기)·S7–S13 = 15건 → S1·S2·S3만 내려감, 가장 오래된 P1과 P2 유지, 목록 최신순 [S3,S2,S1], 문구가 연결 한계를 말함 / P1이 확정된 뒤 다음 조치에서 P1(가장 먼저 보낸 확정)이 내려가고 대기 P2는 유지 | PASS |
| 다시 열기: S1을 id로 새로 읽어 맨 위로, 대신 S4가 내려감 / 503이면 목록에 남고 ‘다시 읽지 못했습니다 (storage)’ | PASS |
| 재연결 시 ‘화면에서 내린 결과’도 비어 있음 | PASS |

**기존 시험(영향).** e2e ops · ops-selection · ops-distribution · ops-help · ops-lesson-settings · ops-roster · report — PASS. chalk typecheck — PASS. Service·확장 소스 변경 없음.
