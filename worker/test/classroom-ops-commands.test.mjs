...
try {
  ...
  await check('AT-33 slice: fresh schema.sql equals the previous schema plus migrations 0011–0016; all re-runnable and additive', async () => {
    ...
    const m = ['0011-classroom-ops', '0012-classroom-ops-commands', '0013-classroom-ops-control', '0014-classroom-ops-evidence-review', '0015-classroom-collection', '0016-classroom-report-jobs'].map((n) => readFileSync(new URL(`../migrations/${n}.sql`, import.meta.url), 'utf8'));
    const upgraded = new DatabaseSync(':memory:'); upgraded.exec(execFileSync('git', ['show', '75fe6e4:worker/schema.sql'], { encoding: 'utf8' })); for (const sql of [...m, ...m]) upgraded.exec(sql);
