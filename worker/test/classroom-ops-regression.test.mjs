...
const OPS_TABLES = ['class_run_ops', 'class_run_seats', 'ops_grants', 'ops_device_connections', 'ops_latest_state', 'ops_events', 'ops_audit', 'ops_token_issues', 'ops_commands', 'ops_command_targets', 'ops_seat_leases', 'class_run_control', 'ops_event_reviews', 'classroom_consents', 'classroom_collect_batches', 'classroom_collect_items', 'classroom_snapshots', 'classroom_job_outbox', 'classroom_collect_tombstones', 'classroom_report_jobs', 'classroom_recipients', 'classroom_delivery_approvals', 'classroom_report_deliveries', 'classroom_report_links', 'classroom_delivery_events'];
{
  ...
  try {
    await check('AT-32 global switch OFF: every new route answers "not enabled", the existing class surface works, and no operations row is ever written', async () => {
      ...
      for (const [path, method, token, headers] of [[R, 'PUT'], [R + '/status', 'GET'], [R + '/pairings', 'POST'], [R + '/commands', 'POST'], [R + '/commands/x', 'GET'], [R + '/control', 'PUT'], [R + '/evidence/a.b.1', 'PUT'], [R + '/report-batches', 'POST'], [B + '/jobs', 'POST'], [B + '/reports', 'GET'], [B + '/runner-grants', 'POST'], [B + '/recipients', 'GET'], [B + '/approve', 'POST'], [B + '/deliver', 'POST'], [B + '/deliveries', 'GET'], ['/v1/classroom/ops/connect', 'POST', null], ['/v1/classroom/ops/sync', 'POST', cred], ['/v1/classroom/ops/collect/consent', 'POST', cred], ['/v1/classroom/ops/runner/claim', 'POST', cred], ['/v1/classroom/report-links/' + 'a'.repeat(64), 'GET', null], ['/admin/classroom/consents', 'POST', null, admin], ['/admin/classroom/recipients', 'POST', null, admin], ['/admin/classroom/delivery-events', 'POST', null, admin]]) { const r = await off.request(path, method, method === 'GET' ? undefined : {}, token === undefined ? off.teacherToken : token, headers ?? {}); assert.equal(r.status, 404, `${method} ${path} → ${r.status}`); }
