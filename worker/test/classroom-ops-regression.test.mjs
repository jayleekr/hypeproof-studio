...
try {
  ...
  await check('AT-32 switch ON but every per-run flag OFF (the default): a configured run observes, commands, collects, reports and sends nothing', async () => {
    assert.equal((await f.request(f.base, 'PUT', { expected_roster_revision: 0, seats: [{ seat_id: 'A1', student_id: 'student-a' }] })).status, 201); assert.deepEqual((await f.request(f.base + '/status')).json.run.flags, { ops_observe: false, ops_commands: false, ops_collect: false, ops_reports: false, ops_delivery: false, ops_distribute: false, ops_lesson_settings: false });
