...
check('class: live opening on its reviewed template passes; lessons read from the template cohort', () => {
  assert.deepEqual(decideCohort(template, { c: 'class-a', lesson: { course_id: 'course-a' } }, binding(), NOW), { ok: true, lessonCohort: 'tpl-cohort' });
});
...
check('negative: token carries another opening\'s course', () => {
  assert.equal(decideCohort(template, { c: 'class-a', lesson: { course_id: 'course-b' } }, binding(), NOW).reason, 'opening_course_mismatch');
});
...
check('acceptance shape: one template, two openings, no copy — each resolves independently', () => {
  const a = decideCohort(template, { c: 'class-a' }, binding(), NOW);
  const b = decideCohort(template, { c: 'class-b' }, binding({ class_cohort: 'class-b', course_id: 'course-b' }), NOW);
  assert.ok(a.ok && b.ok);
  assert.equal(decideCohort(template, { c: 'class-b', lesson: { course_id: 'course-a' } }, binding({ class_cohort: 'class-b', course_id: 'course-b' }), NOW).reason, 'opening_course_mismatch');
});
