...
try {
  ...
  const switchTo = (seat, n, item, expected, extra = {}) => f.request('/v1/classroom/ops/lesson-binding', 'POST', { app_instance_id: f.instance(n).app_instance_id, boot_id: f.instance(n).boot_id, offer_key: item.offer_key, distribution_id: item.distribution_id, object_id: item.object_id, revision: item.revision, content_hash: item.content_hash, expected_binding_seq: expected, base_lesson_sha256: sha[V1], ...extra }, conn[seat].credential);
