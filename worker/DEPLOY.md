...
know 0011–0021; it now has an explicit `apply_classroom_ops_schema` input (default off). `wrangler.toml` has no
...
0011–0021 contain only `CREATE TABLE|INDEX IF NOT EXISTS` (the D1 test asserts this), so order matters only for
...
for f in 0011-classroom-ops 0012-classroom-ops-commands 0013-classroom-ops-control 0014-classroom-ops-evidence-review \
         ...
         0019-classroom-report-attempts 0020-classroom-viewer-check 0021-classroom-erasure-log; do
...
A migration added after 0021 for this feature goes to the end of both lists (here and in `deploy-worker.yml`); the
