...
function buildSkeletonLibrary(profile: Profile, runtime: CoachRuntime = "proxy"): string {
  ...
  if (tier === "kids-quest" && runtime === "sdk") {
    return [
      ...
      "엔진 치트시트(추측 금지): sprite(S_이름,x,y,PAL,scale) 16×16 문자 스프라이트 · x=도트 캔버스 ctx(fillRect) · emo(이모지,px,py,크기) · say(말) · report(r,ok,제목,성공,실패) · rnd() · CONFIG/WORLD/state/loop(). 다리·팔 추가는 새 스프라이트 설계 대신 같은 스프라이트를 옆에 한 번 더 그리거나 fillRect 로 덧그린다. 새 기능 = 배열 하나 + loop 안 3줄.",
