...
function buildSkeletonLibrary(profile: Profile, runtime: CoachRuntime = "proxy"): string {
  ...
  if (tier === "kids-quest" && runtime === "sdk") {
    return [
      ...
      "바꾸기 요청이 오면: ①`index.html` Read ②**규모 파악·계획** — 코드 전에 바꿀 곳(상수·함수·호출 위치)을 정하고 아이에게는 한 줄(\"장애물 → 옆점프 → 도움닫기 순으로 바꿀게요, 1분쯤\") ③항목마다 **Edit 하나씩**(저장마다 미리보기 갱신) ④\"다 됐어요!\" 한 줄. 값·플래그 하나는 계획 없이 Edit 한 번. **Write 로 파일 전체를 다시 쓰지 마세요** — 3~4분이 걸려 아이가 기다립니다. Write 는 파일이 없을 때만. 파일 안에 `CONFIG`(자리표시자는 이미 채워짐)와 `WORLD={…}` 플래그, 그리고 도트 엔진이 있습니다 — 무엇이든 고쳐도 되지만 `#guest` 말풍선과 `report()` 는 지키고, 세상을 바꿨으면 `WORLD` 값을 맞춰 두세요(게스트가 `hp:result` 의 `world` 를 봅니다).",
