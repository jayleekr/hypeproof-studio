...
function buildSkeletonLibrary(profile: Profile, runtime: CoachRuntime = "proxy"): string {
  ...
  if (tier === "kids-quest" && runtime === "sdk") {
    return [
      ...
      "엔진 치트시트(추측 금지): sprite(S_이름,x,y,PAL,scale) 16×16 문자 스프라이트 · x=도트 캔버스 ctx(fillRect) · emo(이모지,px,py,크기) · hud(자리,글,색) · txt(글,y,크기,색) · say(말) · report(r,ok,제목,성공,실패) · rnd() · CONFIG/WORLD/state/loop(). 다리·팔 추가는 새 스프라이트 설계 대신 같은 스프라이트를 옆에 한 번 더 그리거나 fillRect 로 덧그린다. 새 기능 = 배열 하나 + loop 안 3줄.",
      "자리 있는 화면 글자(점수판 등)는 `hud('왼쪽 위', '점수 '+score)` 한 줄 — 자리는 아이 말 그대로(왼쪽 위·오른쪽 위·가운데 위·왼쪽 아래·오른쪽 아래·가운데 아래). 좌표 계산 금지. 매 프레임 값은 loop() 안에서.",
      "답에 `User:` · `assistant:` 같은 라벨을 쓰지 마라 — 대화 기록 표시일 뿐이고 코치는 자기 말만 한다.",
      "한 응답에 Edit 는 최대 3곳 — 더 필요하면 다음 응답에서 이어서(자동으로 차례가 온다). 전체 설계를 머릿속에서 끝내려 오래 생각하지 마라: 실기기에서 10분 침묵이 나왔다. 첫 조각은 60초 안에 화면에.",
      "'되돌려줘'가 오면 새로 만들지 말고 index.html 을 Read 해 직전 변경을 반대로 고친다 — 아이가 말한 단계만큼만, 다른 부분은 건드리지 않는다.",
      "링크(URL)는 열 수 없다 — 추측하지 말고 \"캡처해서 채팅에 붙여넣어 주세요 📸\" 한 줄로 부탁하라. 이미지 붙여넣기는 된다.",
