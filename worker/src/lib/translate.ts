...
function buildSkeletonLibrary(profile: Profile, runtime: CoachRuntime = "proxy"): string {
  ...
  if (tier === "kids-quest" && runtime === "sdk") {
    return [
      ...
      "바꾸기 요청이 오면 **`index.html` 을 한 번 Read 한 뒤, 바뀌는 줄만 `Edit`** 하세요(저장되면 Studio 가 자동으로 다시 띄웁니다). **파일 전체를 `Write` 로 다시 쓰지 마세요** — 세상 하나가 6천 토큰이라 아이가 100초를 기다립니다. `Edit` 이면 3초입니다.",
      "아이 말 → 고칠 곳: 빠르기·개수·목표·특별값은 파일 맨 위 `CONFIG`(`SPEED` `RATE` `GOAL` `SPECIAL`) · 떨어지는 것/받는 것은 `ITEM_A` `ITEM_B` · 하늘색은 `BG_TOP` `BG_BOT` · 세상 상태는 `WORLD={…}` 플래그. 대부분의 요청은 이 몇 줄이면 끝납니다. 여러 개를 한 번에 바꿀 때도 `Edit` 을 몇 번 나눠 부르는 편이 통짜 `Write` 보다 훨씬 빠릅니다.",
      "파일 아래쪽 도트 엔진(스프라이트·그리기 루프)은 **건드리지 마세요** — 아이가 바꾸는 곳이 아닌데 분량은 파일의 4/5 입니다. 정말로 새 오브젝트나 새 규칙이 필요하면 그 함수 **하나만** `Edit` 으로 갈아끼웁니다. `Write` 는 파일이 아예 없을 때만.",
      "무엇이든 고쳐도 되지만 `#guest` 말풍선과 `report()` 는 지키고, 세상을 바꿨으면 `WORLD` 값을 맞춰 두세요(게스트가 `hp:result` 의 `world` 를 봅니다).",
