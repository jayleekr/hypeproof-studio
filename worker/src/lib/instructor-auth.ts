...
export function isIssuerAllowedEndpoint(path: string, method: string): boolean {
  ...
  if (/^\/admin\/cohorts\/[^/]+\/classroom\/runs\/[^/]+(?:\/(?:status|pairings|control|grants\/[^/]+|evidence\/[^/]+|report-batches(?:\/[^/]+(?:\/reconcile)?)?|commands(?:\/[^/]+)?))?$/.test(path) && ['GET', 'PUT', 'POST', 'DELETE'].includes(method)) return true;
