...
export function isIssuerAllowedEndpoint(path: string, method: string): boolean {
  ...
  if (/^\/admin\/cohorts\/[^/]+\/classroom\/runs\/[^/]+(?:\/(?:status|pairings|control|grants\/[^/]+|evidence\/[^/]+|report-batches(?:\/[^/]+(?:\/(?:reconcile|advance|jobs|runner-grants|reports(?:\/[^/]+(?:\/review)?)?|recipients|approve|deliver|deliveries(?:\/[^/]+\/(?:resolve|link))?))?)?|commands(?:\/[^/]+)?))?$/.test(path) && ['GET', 'PUT', 'POST', 'DELETE'].includes(method)) return true;
