...
**가장 좋은 방법은 포트를 찾지 않는 것이다.** 라이브 프리뷰를 직접 시작하는 도구를
가지고 있다면 그것이 완성된 주소를 그대로 돌려준다 — 추측할 필요가 없다. 그 주소의
포트를 쓰면 아래 탐색은 통째로 건너뛴다. 그런 도구가 없고 참가자가 이미 ▶ Run 으로
프리뷰를 띄운 상태라면, **주소를 지어내지 말고 참가자에게 주소창의 주소를 물어라** —
그것도 추측보다 빠르고 정확하다.
